...
@TestPropertySource(properties = {
        ...
        "scalar.authentication.securitySchemes.my-api-key.name=X-API-Key",

        // OAuth2 authentication
        "scalar.authentication.securitySchemes.my-oauth-scheme.flows.authorizationCode.authorizationUrl=https://auth.example.com/oauth/authorize",
        "scalar.authentication.securitySchemes.my-oauth-scheme.flows.authorizationCode.tokenUrl=https://auth.example.com/oauth/token",

        // HTTP Basic authentication
        "scalar.authentication.securitySchemes.my-basic-auth.username=testuser",
        "scalar.authentication.securitySchemes.my-basic-auth.password=testpass",
...
class SpringBootConfigurationBindingTest {
    ...
    void shouldBindAuthenticationProperties() {
        ...
        // assertThat(auth.getSecuritySchemes()).hasSize(2);

        // // Verify API Key security scheme
        // ScalarApiKeySecurityScheme apiKeyScheme = (ScalarApiKeySecurityScheme)
        // auth.getSecuritySchemes()
        // .get("my-api-key");
        // assertThat(apiKeyScheme).isNotNull();
        // assertThat(apiKeyScheme.getName()).isEqualTo("X-API-Key");

        // // Verify OAuth2 security scheme
        // ScalarOAuth2SecurityScheme oauth2Scheme = (ScalarOAuth2SecurityScheme)
        // auth.getSecuritySchemes()
        // .get("my-oauth-scheme");
        ...
        // ScalarHttpSecurityScheme httpScheme = (ScalarHttpSecurityScheme)
        // auth.getSecuritySchemes().get("my-basic-auth");
