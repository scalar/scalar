...
describe("scalar.aspnetcore", () => {
  ...
  describe("initialize", () => {
    ...
    it("transforms URLs to absolute paths when using relative URLs", async () => {
      ...
      expect(normalizedConfig.sources[0].url).toBe(
        "https://example.com/api/swagger.json",
      );
      expect(normalizedConfig.sources[1].url).toBe(
        "https://example.com/api/openapi.json",
      );
      expect(mockScalar.createApiReference).toHaveBeenCalledWith(
        "#app",
        normalizedConfig,
      );
    });
    ...
    it("preserves URLs when using absolute paths", async () => {
      const configuration = {
        sources: [
          { url: "https://foo.bar/swagger.json" },
          { url: "htTP://foo.bar/openapi.json" },
        ],
      };
    ...
    it("handles sources without url property", async () => {
      ...
      expect(normalizedConfig.sources[0]).toEqual({
