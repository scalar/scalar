...
const emit = defineEmits<{
  (e: 'input', value: { source: string; type: 'file' | 'raw' }): void
}>()
...
const isFromApp = (event: DragEvent): boolean => {
...
const handleDrop = (event: DragEvent) => {
  ...
  if (droppedText) {
    return emit('input', { source: droppedText, type: 'raw' })
  }
  ...
  // For Electron environments, get the file path and emit it as the source
  if (window.electron === true) {
    const filePath = window.api.getPathForFile(file)
    return emit('input', { source: filePath, type: 'file' })
  }
  // For browser environments, read the file as text and emit it as the raw source
  const reader = new FileReader()

  reader.onload = (e) => {
    if (e.target && typeof e.target.result === 'string') {
      emit('input', { source: e.target.result, type: 'raw' })
    }
  }
