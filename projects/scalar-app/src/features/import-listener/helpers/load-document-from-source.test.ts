...
describe('loadDocumentFromSource', () => {
  ...
  it('adds document from URL with watch mode enabled', async () => {
    const workspaceStore = createWorkspaceStore()
    ...
    const result = await loadDocumentFromSource(
      workspaceStore,
      { type: 'url', source: 'https://example.com/openapi.json' },
      'URL API',
      true,
    )
    ...
    const addedDocument = documents.find((doc) => doc.info.title === 'URL API')

    expect(addedDocument).toBeDefined()
    expect(addedDocument?.['x-scalar-watch-mode']).toBe(true)
  })

  it('adds document from URL with watch mode disabled', async () => {
    const workspaceStore = createWorkspaceStore()
    ...
    global.fetch = vi.fn().mockResolvedValue({
      ...
      text: () =>
        Promise.resolve({
          openapi: '3.1.0',
          info: { title: 'Another URL API', version: '1.0.0' },
        }),
    })

    const result = await loadDocumentFromSource(
      workspaceStore,
      { type: 'url', source: 'https://example.com/openapi.yaml' },
      'Another URL API',
      false,
    )
  ...
  it('adds document from file path', async () => {
    ...
    const result = await loadDocumentFromSource(
      workspaceStore,
      { type: 'file', source: '/path/to/openapi.json' },
      'File API',
      false,
    )
    ...
    const documents = Object.values(workspaceStore.workspace.documents)
    const addedDocument = documents.find((doc) => doc.info.title === 'File API')
...
`
  ...
  it('handles complex OpenAPI document with multiple operations', async () => {
    ...
    const result = await loadDocumentFromSource(
      workspaceStore,
      { type: 'raw', source: complexOpenAPI },
      'Complex API',
      false,
    )
  ...
  it('converts and adds valid Postman collection', async () => {
    ...
    const result = await loadDocumentFromSource(
      workspaceStore,
      { type: 'raw', source: postmanCollection },
      'My Postman API',
      false,
    )
  ...
  it('handles Postman collection with multiple requests', async () => {
    ...
    const result = await loadDocumentFromSource(
      workspaceStore,
      { type: 'raw', source: complexPostmanCollection },
  ...
  it('returns false when Postman conversion fails', async () => {
    ...
    const result = await loadDocumentFromSource(
      workspaceStore,
      { type: 'raw', source: invalidPostmanCollection },
      'Invalid Collection',
      false,
    )
    expect(result).toBe(false)
  ...
  it('handles empty Postman collection', async () => {
    ...
    const result = await loadDocumentFromSource(
      workspaceStore,
      { type: 'raw', source: emptyPostmanCollection },
  ...
  it('handles Postman collection with authentication', async () => {
    ...
    const result = await loadDocumentFromSource(
      workspaceStore,
      { type: 'raw', source: postmanWithAuth },
      'Authenticated API',
      false,
    )
  ...
  it('returns false for invalid content that cannot be normalized', async () => {
    ...
    const result = await loadDocumentFromSource(
      workspaceStore,
      { type: 'raw', source: invalidContent },
      'Invalid Doc',
      false,
    )
  ...
  it('handles Postman collection with variables', async () => {
    ...
    const result = await loadDocumentFromSource(
      workspaceStore,
      { type: 'raw', source: postmanWithVariables },
      'API with Variables',
      false,
    )
