...
type TrackingContext = NonNullable<
  Parameters<typeof initializeWebsiteTrackers>[0]
>
...
describe('initializeWebsiteTrackers', () => {
  ...
  it('loads Apollo and Vector on client.scalar.com', () => {
    ...
    const targetDocument =
      document.implementation.createHTMLDocument('tracking test')
  ...
  it('does not inject duplicate trackers when already initialized', () => {
    ...
    expect(
      targetWindow.vector?.q.filter(([method]) => method === 'load'),
    ).toHaveLength(1)
  })
})
