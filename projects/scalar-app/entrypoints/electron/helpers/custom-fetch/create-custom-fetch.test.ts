...
let mockTransport: ReturnType<
  typeof vi.fn<(request: IpcFetchRequest) => Promise<TransportResponse>>
>
let customFetch: ReturnType<typeof createCustomFetch>

beforeEach(() => {
  mockTransport =
    vi.fn<(request: IpcFetchRequest) => Promise<TransportResponse>>()
...
describe('create-custom-fetch', () => {
  describe('wiring', () => {
    ...
    it('returns a DOM Response built from the transport result', async () => {
      mockTransport.mockResolvedValue(
        createMockIpcResponse({ status: 418, statusText: "I'm a teapot" }),
      )
  ...
  describe('error handling', () => {
    it('propagates network errors thrown by the transport', async () => {
      ...
      await expect(customFetch('https://example.com')).rejects.toThrow(
        'fetch failed',
      )
    })

    it('propagates DOMException AbortError', async () => {
      mockTransport.mockRejectedValue(
