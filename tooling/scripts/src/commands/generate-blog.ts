...
async function updateIndex(root: string, indexPath: string, posts: BlogPost[]): Promise<void> {
  ...
  const itemsMarkup = postsWithDescriptions.map((post) => formatPost(post)).join('\n')
  const newContent = replaceCardsSection(indexContent, itemsMarkup)
...
async function parseBlogPost(blogDir: string, filename: string): Promise<BlogPost> {
  ...
  return { filename, slug, title, date, dateLabel, isoDate, description }
}
...
function formatPost(post: BlogPost): string {
  ...
  <a class="blog-post-link" href="./${post.filename}">
    <time class="blog-post-date" datetime="${post.isoDate}">${post.dateLabel}</time>
    <div class="blog-post-body">
      <h2 class="blog-post-title">${escapedTitle}</h2>
...
/**
 * Parse existing post descriptions from the rendered list. Used to preserve
 * any hand-edited descriptions when regenerating the index.
 */
function parseExistingDescriptions(indexContent: string): Map<string, string> {
  const map = new Map<string, string>()
  const postBlock =
    /<a class="blog-post-link" href="\.\/([^"]+)">[\s\S]*?<p class="blog-post-description">([\s\S]*?)<\/p>/g
  let match: RegExpExecArray | null
  while ((match = postBlock.exec(indexContent)) !== null) {
    const filename = match[1]
    const description = match[2]?.trim()
    if (filename && description) {
      map.set(filename, unescapeHtml(description))
    }
  }
  return map
}
