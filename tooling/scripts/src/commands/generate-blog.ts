...
async function updateIndex(root: string, indexPath: string, posts: BlogPost[]): Promise<void> {
  ...
  const tableMarkdown = formatTable(postsWithDescriptions)
  const newContent = replaceCardsSection(indexContent, tableMarkdown)
...
function formatTable(posts: BlogPost[]): string {
  const rows = posts
    .map((post) => {
      const escapedDesc = post.description
        ...
        .replace(/\[([^\]]*)\]\([^)]*\)/g, '$1')
        .replace(/`([^`]*)`/g, '$1')
      return `<!-- desc:${post.filename}:${escapedDesc} -->
<a class="blog-row" href="./${post.filename}">
  <span class="blog-date">${post.dateLabel}</span>
...
/**
 * Parse existing descriptions from HTML comment markers or legacy card format.
 * New format: `<!-- desc:filename.md:description text -->`
 * Legacy format: `:::scalar-card{...href="./filename.md"...}\n...\n::scalar-fineprint`
 */
function parseExistingDescriptions(indexContent: string): Map<string, string> {
  const map = new Map<string, string>()

  const commentBlock = /<!-- desc:([^:]+):([\s\S]*?) -->/g
  let match: RegExpExecArray | null
  while ((match = commentBlock.exec(indexContent)) !== null) {
