...
describe("formatter helpers", () => {
  it("formats release line with PR link", () => {
    ...
    const result = formatReleaseLine(
      changeset as NewChangesetWithCommit,
      githubInfo,
    );

    expect(result).toBe(
      "- [#123](https://github.com/scalar/scalar/pull/123): Do something great",
    );
  });
  ...
  it("formats dependency header and change", () => {
    expect(formatDependencyHeader("@scalar/api-reference", "1.2.3")).toBe(
