...
describe("normalizeAuthSchemes", () => {
  ...
  it("does not normalize scheme at wrong depth", async () => {
    ...
    expect(input.components.securitySchemes.bearerAuth.nested.scheme).toBe(
      "Bearer",
    );
  });
...
describe("normalizeRefs", () => {
  ...
  it("normalizes $ref in request body", async () => {
    ...
    expect(
      input.paths["/users"].post.requestBody.content["application/json"].schema,
    ).toEqual({
      $ref: "#/components/schemas/User",
      summary: undefined,
      description: undefined,
  ...
  it("normalizes $ref in headers", async () => {
    ...
    expect(
      input.paths["/users"].get.responses["200"].headers["X-Rate-Limit"],
    ).toEqual({
      $ref: "#/components/headers/RateLimit",
      summary: undefined,
      description: undefined,
...
describe("syncPathParameters", () => {
  ...
  it("creates path parameters for multiple variables", async () => {
    ...
    expect(
      input.paths["/users/{userId}/posts/{postId}"].get.parameters,
    ).toHaveLength(2);
    expect(
      input.paths["/users/{userId}/posts/{postId}"].get.parameters,
    ).toEqual([
  ...
  it("preserves non-path parameters", async () => {
    ...
    expect(input.paths["/users/{id}"].get.parameters[1]).toMatchObject({
      name: "format",
      in: "query",
    });
    expect(input.paths["/users/{id}"].get.parameters[2]).toMatchObject({
      name: "Authorization",
      in: "header",
    });
    expect(input.paths["/users/{id}"].get.parameters[3]).toMatchObject({
      name: "session",
      in: "cookie",
    });
  });
  ...
  it("removes path parameters that no longer exist in the path", async () => {
    ...
    expect(input.paths["/users"].get.parameters[0]).toMatchObject({
      name: "format",
      in: "query",
    });
  });
  ...
  it("adds new path parameters while preserving existing ones", async () => {
    ...
    expect(
      input.paths["/users/{userId}/posts/{postId}"].get.parameters[0],
    ).toMatchObject({
      name: "userId",
      in: "path",
      description: "User ID",
    ...
    expect(
      input.paths["/users/{userId}/posts/{postId}"].get.parameters[1],
    ).toMatchObject({
      name: "postId",
      in: "path",
    });
  ...
  it("handles $ref parameters", async () => {
    ...
    expect(
      input.paths["/users/{id}"].get.parameters.length,
    ...
    const refParams = input.paths["/users/{id}"].get.parameters.filter(
      (p: any) =>
        typeof p?.$ref === "string" &&
        p.$ref === "#/components/parameters/IdParam",
    );
    expect(refParams).toHaveLength(1);

    // Should have the query parameter
    const queryParams = input.paths["/users/{id}"].get.parameters.filter(
      (p: any) => p.in === "query",
    );
  ...
  it("keeps operation-level referenced path parameters as $ref entries", async () => {
    ...
    expect(input.paths["/users/{id}"].get.parameters[0]).not.toHaveProperty(
      "name",
    );
  });
  ...
  it("keeps path-item referenced path parameters as $ref entries when operation does not define them", async () => {
    ...
    expect(input.paths["/users/{id}"].get.parameters[1]).not.toHaveProperty(
      "name",
    );
  });
  ...
  it("handles multiple referenced parameters", async () => {
    ...
    const refParams = input.paths[
      "/users/{userId}/posts/{postId}"
    ].get.parameters.filter((p: any) => typeof p?.$ref === "string");
  ...
  it("handles mix of referenced and inline path parameters", async () => {
    ...
    const refParam = input.paths[
      "/users/{userId}/posts/{postId}"
    ].get.parameters.find(
      (p: any) => p.$ref === "#/components/parameters/UserIdParam",
    );
    const postIdParam = input.paths[
      "/users/{userId}/posts/{postId}"
    ].get.parameters.find((p: any) => p.in === "path" && p.name === "postId");
  ...
  it("uses path-item $ref parameter when operation does not declare it", async () => {
    ...
    const params = input.paths["/users/{id}"].get.parameters;
    const pathParamRef = params.find(
      (p: any) => p.$ref === "#/components/parameters/IdParam",
    );
  ...
  it("does not duplicate when operation $ref and path-item inline resolve to the same param", async () => {
    ...
    const inlinePathParams = params.filter(
      (p: any) => p.in === "path" && p.name === "id",
    );
