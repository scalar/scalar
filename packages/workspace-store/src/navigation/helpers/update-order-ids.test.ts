...
const makeWebhookEntry = (
  overrides: Partial<WithParent<TraversedWebhook>> = {},
): WithParent<TraversedWebhook> => ({
  id: "webhook-1",
  type: "webhook",
  title: "User created",
  ref: "webhook-ref",
...
describe("update-order-ids", () => {
  describe("updateOrderIds – operation entries", () => {
    it("replaces the entry ID in x-scalar-order of a document parent", () => {
      ...
      expect(store.workspace.documents[docName]["x-scalar-order"]).toEqual([
        "op-new",
        "op-2",
      ]);
    ...
    it("passes the correct parentTag when the parent entry is a tag", () => {
      ...
      expect(
        store.workspace.documents[docName].tags![0]!["x-scalar-order"],
      ).toEqual(["op-updated"]);
      expect(generateId).toHaveBeenCalledWith(
        expect.objectContaining({
          type: "operation",
  ...
  describe("updateOrderIds – tag entries", () => {
    ...
    it("rewrites child IDs in the tag x-scalar-order when the tag ID changes", () => {
        ...
        tags: [
          {
            name: "customers",
            "x-scalar-order": [
              "test-api/tag/users/GET/users",
              "test-api/tag/users/POST/users",
            ],
          },
        ],
      ...
      expect(tag?.["x-scalar-order"]).toEqual([
        "test-api/tag/customers/GET/users",
        "test-api/tag/customers/POST/users",
      ]);
    });
    ...
    it("skips the rewrite when the direct parent is a tagGroup (getOpenapiObject returns null for x-tagGroups)", () => {
      ...
      store.workspace.documents[docName] = {
        ...
        info: { title: "Test API", version: "1.0.0" },
        tags: [
          { name: "customers", "x-scalar-order": [...originalChildOrder] },
        ],
        "x-scalar-order": ["test-api/tag/users"],
        "x-scalar-original-document-hash": "abc",
      };
    ...
    it("skips child order rewrite when the tag has no x-scalar-order", () => {
      ...
      expect(store.workspace.documents[docName].tags?.[0]).not.toHaveProperty(
        "x-scalar-order",
      );
    });
  });
