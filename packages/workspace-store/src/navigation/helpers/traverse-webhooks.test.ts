...
describe('traverse-webhooks', () => {
  describe('traverseWebhooks', () => {
    ...
    it('marks webhooks with required security', () => {
      const document: OpenApiDocument = {
        openapi: '3.1.0',
        info: { title: 'Test API', version: '1.0.0' },
        paths: {},
        webhooks: {
          'secure-webhook': {
            post: {
              ...
              operationId: 'secureWebhook',
              security: [{ webhookApiKey: [] }],
            },
          },
        },
        'x-scalar-original-document-hash': '',
      ...
      expect(result[0]?.hasSecurityRequirements).toBe(true)
    })

    it('does not mark webhooks when auth is optional', () => {
      const document: OpenApiDocument = {
        openapi: '3.1.0',
        info: { title: 'Test API', version: '1.0.0' },
        paths: {},
        ...
        webhooks: {
          'optional-auth-webhook': {
            post: {
              summary: 'Optional Auth Webhook',
              operationId: 'optionalWebhook',
            },
          },
        },
        'x-scalar-original-document-hash': '',
      ...
      expect(result).toHaveLength(1)
      expect(result[0]?.hasSecurityRequirements).toBeUndefined()
    })
