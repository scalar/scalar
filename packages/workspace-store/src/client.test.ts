...
describe('create-workspace-store', () => {
  ...
  it('infers anyOf from discriminator mapping when explicit variants are missing', async () => {
    const store = createWorkspaceStore()
    await store.addDocument({
      name: 'default',
      document: {
    ...
    const shape = getRaw(store.workspace.activeDocument?.components?.schemas?.['Shape'])
