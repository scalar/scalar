...
describe("updateSelectedSecuritySchemes", () => {
  it("initializes document x-scalar-selected-security and sets selected index to 0 when schemes exist", async () => {
    ...
    expect(
      store.auth.getAuthSelectedSchemas({ type: "document", documentName }),
    ).toEqual({
      selectedIndex: 0,
  ...
  it("appends newly created schemes with unique names and updates selection", async () => {
    ...
    const securitySchemes =
      store.workspace.activeDocument?.components?.securitySchemes;
...
describe("clearSelectedSecuritySchemes", () => {
  ...
  it("clears operation-level selected schemes when meta.type is operation", async () => {
    ...
    mutators.clearSelectedSecuritySchemes({
      meta: { type: "operation", path: "/pets", method: "get" },
    });

    expect(
      store.auth.getAuthSelectedSchemas({
        type: "operation",
        documentName,
        path: "/pets",
        method: "get",
...
describe("updateSecurityScheme", () => {
  it("updates apiKey scheme name", () => {
    ...
    const scheme = document.components!.securitySchemes!.ApiKey as Record<
      string,
      unknown
    >;
  ...
  it("updates oauth2 authorizationCode flow fields", () => {
    ...
    const flow = getResolvedRef(
      document.components?.securitySchemes?.OAuth as OAuth2Object,
  ...
  it("updates openIdConnect discovery url", () => {
    ...
    updateSecurityScheme(document, {
      ...
      payload: {
        type: "openIdConnect",
        openIdConnectUrl:
          "https://new-issuer.example.com/.well-known/openid-configuration",
      },
    });

    const scheme = document.components!.securitySchemes!
      .OpenIDConnect as Record<string, unknown>;
...
describe("clearSecuritySchemeSecrets", () => {
  ...
  it("clears secrets for a security scheme through authMutatorsFactory", async () => {
    ...
    expect(
      store.auth.getAuthSecrets(documentName, "OpenIDConnect"),
    ).toBeTruthy();
...
describe("updateSelectedAuthTab", () => {
  ...
  it("is a no-op when document is null", () => {
    updateSelectedAuthTab(createWorkspaceStore(), null, {
      index: 1,
      meta: { type: "document" },
    });
  });
...
describe("updateSelectedScopes", () => {
  it("updates scopes for the matching scheme at document level", async () => {
    ...
    const schemes = store.auth.getAuthSelectedSchemas({
      type: "document",
      documentName,
    });
    assert(schemes);
  ...
  it("updates scopes for matching scheme at operation level", async () => {
    ...
    const docSchemes = store.auth.getAuthSelectedSchemas({
      type: "document",
      documentName,
    });
    assert(docSchemes);
  ...
  it("matches by id array of keys (multi-scheme requirement) and updates only the named entry", async () => {
    ...
    const scheme = store.auth.getAuthSelectedSchemas({
      type: "document",
      documentName,
    });
    assert(scheme);
  ...
  it("is a no-op when document is null", () => {
    updateSelectedScopes(createWorkspaceStore(), null, {
      id: ["x"],
      name: "x",
      scopes: [],
      meta: { type: "document" },
  ...
  it("is a no-op when selected schemes are missing or scheme id not found", async () => {
    ...
    const zSchemes = store.auth.getAuthSelectedSchemas({
      type: "document",
      documentName,
    });
    assert(zSchemes);
...
describe("deleteSecurityScheme", () => {
  it("removes schemes from components, document selections, document security, and operations", async () => {
    ...
    deleteSecurityScheme(store, store.workspace.activeDocument!, {
      names: ["A", "B", "X"],
    });
    ...
    const op = getResolvedRef(
      store.workspace.activeDocument?.paths!["/p"]?.get,
    );
    assert(op);
