...
describe("fetchRequestToHar", () => {
  ...
  it("handles file uploads in form data", async () => {
    ...
    formData.append(
      "avatar",
      new File(["file content"], "avatar.png", { type: "image/png" }),
    );
  ...
  it("handles query parameters with special characters", async () => {
    const request = new Request(
      "https://api.example.com/search?q=hello%20world&filter=type%3Duser",
    );
  ...
  it("handles query parameters with empty values", async () => {
    const request = new Request(
      "https://api.example.com/users?active=&premium=",
    );
  ...
  it("handles duplicate query parameters", async () => {
    const request = new Request(
      "https://api.example.com/users?tag=javascript&tag=typescript&tag=vue",
    );
