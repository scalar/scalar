...
describe("harToOperation", () => {
  ...
  it("converts query parameters from HAR to operation examples", () => {
    ...
    expect(getResolvedRef(resolvedPageParam.examples?.default)?.value).toBe(
      "1",
    );
    expect(
      getResolvedRef(resolvedPageParam.examples?.default)?.["x-disabled"],
    ).toBe(false);
    ...
    expect(getResolvedRef(resolvedLimitParam.examples?.default)?.value).toBe(
      "10",
    );
    expect(
      getResolvedRef(resolvedLimitParam.examples?.default)?.["x-disabled"],
    ).toBe(false);
  });
  ...
  it("converts headers from HAR to operation examples", () => {
    ...
    expect(getResolvedRef(resolvedAuthParam.examples?.example1)?.value).toBe(
      "Bearer token123",
    );
    expect(
      getResolvedRef(resolvedAuthParam.examples?.example1)?.["x-disabled"],
    ).toBe(false);
  });
  ...
  it("converts cookies from HAR to operation examples", () => {
    ...
    expect(getResolvedRef(resolvedSessionParam.examples?.default)?.value).toBe(
      "sess_abc123",
    );
    expect(
      getResolvedRef(resolvedSessionParam.examples?.default)?.["x-disabled"],
    ).toBe(false);
  });
  ...
  it("converts JSON request body from HAR to operation examples", () => {
    ...
    expect(getResolvedRef(jsonContent.examples?.default)?.["x-disabled"]).toBe(
      false,
    );
  });
  ...
  it("converts form data from HAR to operation examples as array", () => {
    ...
    assert(result.requestBody && "content" in result.requestBody);
    const formContent =
      result.requestBody.content?.["application/x-www-form-urlencoded"];
    assert(formContent);
  ...
  it("merges with base operation without overwriting existing parameters", () => {
    ...
    expect(getResolvedRef(resolvedPageParam.examples?.newExample)?.value).toBe(
      "1",
    );
  });
  ...
  it("sets x-scalar-selected-content-type when adding request body", () => {
    ...
    expect(result.requestBody["x-scalar-selected-content-type"]?.example1).toBe(
      "application/json",
    );
  });
  ...
  it("handles path variables in base operation", () => {
    ...
    expect(getResolvedRef(resolvedUserIdParam.examples?.default)?.value).toBe(
      "123",
    );
    expect(
      getResolvedRef(resolvedUserIdParam.examples?.default)?.["x-disabled"],
    ).toBe(false);
  });
  ...
  it("handles multiple path variables", () => {
    ...
    assert(
      resolvedUserId && "schema" in resolvedUserId && resolvedUserId.examples,
    );
    expect(getResolvedRef(resolvedUserId.examples.default)?.value).toBe("123");

    const resolvedPostId = getResolvedRef(postIdParam);
    assert(
      resolvedPostId && "schema" in resolvedPostId && resolvedPostId.examples,
    );
  ...
  it("sets parameters from base operation as disabled when no matching HAR data", () => {
    ...
    expect(getResolvedRef(pageParam.examples.default)?.["x-disabled"]).toBe(
      true,
    );
  });
});
