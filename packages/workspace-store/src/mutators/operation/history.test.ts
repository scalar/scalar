...
} from "@/mutators/operation/history";
...
describe("addResponseToHistory", () => {
  ...
  it("initializes history array when it does not exist", async () => {
    ...
    const updatedHistory = store.history.getHistory(
      "test-doc",
      "/products",
      "post",
    );
              ...
              parameters: [
                {
                  name: "userId",
                  in: "path",
                  examples: { default: { value: "456" } },
                },
                {
                  name: "postId",
                  in: "path",
                  examples: { default: { value: "789" } },
                },
              ],
            },
          },
...
describe("reloadOperationHistory", () => {
  ...
  it("handles history with path variables", async () => {
    ...
    const operation = getResolvedRef(
      document.paths?.["/orders/{orderId}"]?.get,
    );
