...
describe("upsertOperationParameter", () => {
  ...
  it("updates an existing query parameter by passing originalParameter", () => {
          ...
          get: {
            parameters: [
              { name: "X-Trace", in: "header" },
              {
                name: "q",
                in: "query",
                examples: { default: { value: "one" } },
              },
              {
                name: "p",
                in: "query",
                examples: { default: { value: "two" } },
              },
            ],
          },
        },
    ...
    expect(getResolvedRef(secondQuery.examples.default)?.["x-disabled"]).toBe(
      false,
    );
  });
  ...
  it("only updates the specific parameter passed as originalParameter", () => {
    ...
    expect(
      getResolvedRef(updatedQueryParam?.examples?.default as any)?.value,
    ).toBe("1");
  });
  ...
  it("adds multiple parameters of different types", () => {
    ...
    // Note: When adding a new parameter, x-disabled is always set to false initially
    expect(getResolvedRef(params[2].examples.default)?.["x-disabled"]).toBe(
      false,
    );
  });
          ...
          get: {
            parameters: [
              ...
              },
              {
                ...
                in: "query",
                required: false,
                examples: { default: { value: "one", "x-disabled": false } },
              },
              {
                name: "page",
                in: "query",
              ...
              },
              {
                ...
                in: "path",
                required: true,
                examples: { default: { value: "123", "x-disabled": false } },
              },
            ],
          },
        },
...
describe("updateOperationExtraParameters", () => {
  it("sets isDisabled for a default header parameter", () => {
    ...
    expect(
      operation?.["x-scalar-disable-parameters"]?.["default-headers"]?.default
        ?.Authorization,
    ).toBe(true);
  });
  ...
  it("sets isDisabled for a global cookie parameter", () => {
    ...
    const operation = getResolvedRef(document.paths?.["/users"]?.get);
    expect(
      operation?.["x-scalar-disable-parameters"]?.["global-cookies"]?.default
        ?.session,
    ).toBe(true);
  });
  ...
  it("initializes x-scalar-disable-parameters when it does not exist", () => {
    ...
    expect(
      operation["x-scalar-disable-parameters"]?.["default-headers"]?.example1?.[
        "X-API-Key"
      ],
    ).toBe(false);
  });
  ...
  it("handles multiple example keys independently", () => {
    ...
    expect(
      operation?.["x-scalar-disable-parameters"]?.["default-headers"]?.example2
        ?.Authorization,
    ).toBe(false);
  });
