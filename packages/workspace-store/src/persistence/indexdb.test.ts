...
describe("indexdb", () => {
  ...
  beforeEach(async () => {
    dbConnection = await createIndexDbConnection({
      ...
      tables: {
        ...
        orders: {
          schema: OrderSchema,
          keyPath: ["userId", "orderId"] as const,
        },
      },
    });
  ...
  it("adds and retrieves a single item", async () => {
    const user = await dbConnection
      .get("users")
      .addItem({ id: "user-1" }, { name: "Alice", age: 30 });
  ...
  it("updates an existing item when adding with same key", async () => {
    ...
    await dbConnection
      .get("users")
      .addItem({ id: "user-1" }, { name: "Alice Updated", age: 31 });
  ...
  it("retrieves range with composite key - partial match", async () => {
    ...
    await ordersTable.addItem(
      { userId: "user-1", orderId: "order-1" },
      { amount: 100, status: "pending" },
    );
    await ordersTable.addItem(
      { userId: "user-1", orderId: "order-2" },
      { amount: 200, status: "completed" },
    );
    await ordersTable.addItem(
      { userId: "user-2", orderId: "order-1" },
  ...
  it("deletes item with composite key", async () => {
    ...
    const deleted = await ordersTable.getItem({
      userId: "user-1",
      orderId: "order-1",
    });
    const existing = await ordersTable.getItem({
      userId: "user-1",
      orderId: "order-2",
    });
  ...
  it("does not error when deleting non-existent item", async () => {
    ...
    await expect(
      usersTable.deleteItem({ id: "non-existent" }),
