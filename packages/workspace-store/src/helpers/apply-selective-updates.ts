...
import { apply, diff, DifferenceResult } from '@scalar/json-magic/diff'
...
export const applySelectiveUpdates = (originalDocument: UnknownObject, updatedDocument: UnknownObject) => {
  ...
  const [writableDiffs, excludedDiffs] = split(diffs.changeset, (d) => !d.path.some((p) => excludeKeys.has(p)))

  apply(originalDocument, new DifferenceResult(writableDiffs))
