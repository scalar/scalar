...
describe('migrate-to-indexdb', () => {
  ...
  describe('transformLegacyDataToWorkspace - Requests & RequestExamples', () => {
    ...
    describe('adversarial data - breaking the migration', () => {
      it('survives completely mangled data types across all fields', async () => {
        /**
         * Every field is the wrong type: numbers where strings go, strings where arrays go,
         * null where objects go, booleans where arrays go, etc.
         * This simulates severely corrupted localStorage data.
         */
        const garbage = {
          arrays: {
            workspaces: [
              {
                uid: 99999,
                name: null,
                collections: [12345, undefined, null, false, { oops: true }],
                environments: 'not-an-object',
                cookies: false,
                proxyUrl: { nested: { deeply: 'wrong' } },
                themeId: [1, 2, 3],
              },
            ],
          },
          records: {
            collections: {
              12345: {
                ...
                webhooks: false,
                externalDocs: [],
                'x-scalar-environments': 123,
                'x-scalar-icon': { not: 'a-string' },
                useCollectionSecurity: 'yes',
                selectedServerUid: { wrong: true },
                documentUrl: 12345,
              },
            },
            cookies: null,
            environments: undefined,
            requestExamples: 'not-a-record',
            requests: [],
        ...
        /**
         * The migration should either produce a result (graceful degradation) or throw.
         * Either outcome is acceptable, but it must not hang or cause an unhandled rejection.
         */
        let threw = false
        try {
          const result = await transformLegacyDataToWorkspace(garbage)

          // If it somehow succeeds, ensure we get an array back
        ...
        // We just need to confirm it terminated cleanly (threw or returned)
        expect(typeof threw).toBe('boolean')
      })

      it('survives structurally valid data with poisoned nested values', async () => {
        /**
         * The data has the correct shape (arrays are arrays, objects are objects) but all the
         * values inside are designed to break: empty strings, NaN, Infinity, __proto__ keys,
         * massive strings, emoji-only paths, null entries in arrays, and circular-ish structures.
         *
         * This simulates partially corrupted data that passed initial structure validation
         * but has garbage values deep inside.
         */
        const poisonedWorkspace = {
          uid: '__proto__',
          name: '\0\0\0',
          collections: ['col-null', 'col-poison', 'nonexistent', '', null],
          environments: {
            '': '',
            [Symbol.iterator]: 'gotcha',
            __proto__: { polluted: true },
            'normal-looking': { but: undefined },
          },
          cookies: ['missing-cookie-1', null, undefined, ''],
          proxyUrl: 'javascript:alert(1)',
          themeId: 'definitely-not-a-theme',
        }

        const poisonedCollection = {
          uid: 'col-poison',
          openapi: '',
          info: { title: '', version: Number.NaN },
          servers: ['srv-poison', null, undefined],
          requests: ['req-poison', null, ''],
          tags: ['tag-poison', 'tag-circular'],
          children: ['tag-poison'],
          securitySchemes: ['sec-poison', '', null],
          security: [null, undefined, { broken: true }],
          ...
          'x-scalar-environments': {
            Production: {
              ...
              variables: {
                normal: 'value',
                broken: undefined,
                nested: { deep: { value: 'not-a-string' } },
                '': '',
              },
            },
          },
          'x-scalar-icon': '',
          useCollectionSecurity: undefined,
          selectedServerUid: 'srv-poison',
          documentUrl: '',
        ...
        const poisonedRequest = {
          ...
          parameters: [null, undefined, { name: '', in: '' }],
          examples: ['ex-poison', null],
          servers: ['srv-poison'],
          selectedServerUid: 'nonexistent',
          selectedSecuritySchemeUids: [null, '', undefined],
          operationId: '',
          description: '\uD800',
          requestBody: {
            content: {
        ...
        const poisonedRequestExample = {
          uid: 'ex-poison',
          name: '',
          requestUid: 'req-poison',
          parameters: {
            path: [{ key: '', value: null, enabled: 'not-bool' }],
            query: [null, { key: undefined, value: Number.NaN, enabled: undefined }],
            headers: [{ key: '__proto__', value: '{}', enabled: true }],
        ...
        const circularTag = {
          ...
          name: 'Circular',
          description: '',
          children: ['tag-poison', 'tag-circular'],
        }

        const poisonedSecurityScheme = {
          uid: 'sec-poison',
          nameKey: '',
          type: '',
          flows: null,
          'x-default-scopes': 'not-an-array',
        }

        const garbage = {
          arrays: {
            workspaces: [poisonedWorkspace],
            collections: [{ uid: 'col-null' }, poisonedCollection],
          },
          records: {
            collections: {
              'col-null': null,
              'col-poison': poisonedCollection,
            },
            cookies: {},
            environments: {},
            requestExamples: {
              'ex-poison': poisonedRequestExample,
            },
            requests: {
              'req-poison': poisonedRequest,
            },
            securitySchemes: {
              'sec-poison': poisonedSecurityScheme,
            },
            servers: {
              'srv-poison': poisonedServer,
            },
            tags: {
              'tag-poison': poisonedTag,
              'tag-circular': circularTag,
            },
            workspaces: {},
          },
        } as any
        ...
        /**
         * With a valid-ish structure, the migration should get further into the transform.
         * We accept either a graceful result or a thrown error, but no hang or unhandled rejection.
         */
        let threw = false
        try {
          const result = await transformLegacyDataToWorkspace(garbage)

          // If the migration survived, verify basic structural integrity
          expect(Array.isArray(result)).toBe(true)
          expect(result.length).toBe(1)

          // The workspace name should fall back since we gave it a null-byte string
          const ws = result[0]!
          expect(typeof ws.name).toBe('string')
          expect(typeof ws.slug).toBe('string')
          expect(ws.workspace).toBeDefined()
        } catch {
          threw = true
        }
