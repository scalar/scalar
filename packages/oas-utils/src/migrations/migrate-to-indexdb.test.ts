...
import {
  type Workspace,
  workspaceSchema,
} from "@/entities/workspace/workspace";
...
describe("migrate-to-indexdb", () => {
  ...
  describe("shouldMigrateToIndexDb", () => {
    it("returns false when no legacy data exists", async () => {
      ...
      const result = await shouldMigrateToIndexDb(
        mockWorkspacePersistence as any,
      );
    ...
    it("returns false when legacy data exists but IndexedDB already has workspaces", async () => {
      ...
      const mockWorkspacePersistence = {
        getAll: async () => [
          { namespace: "local", slug: "existing-workspace" },
        ],
      };
      const result = await shouldMigrateToIndexDb(
        mockWorkspacePersistence as any,
    ...
    it("handles IndexedDB being cleared after migration - migration runs again", async () => {
      ...
      const shouldMigrateFirst = await shouldMigrateToIndexDb(
        emptyPersistence as any,
      );
      ...
      const shouldMigrateAfterClear = await shouldMigrateToIndexDb(
        clearedPersistence as any,
      );
  ...
  describe("transformLegacyDataToWorkspace - Security Schemes", () => {
    it("should transform API key security schemes into document components and auth store", async () => {
      ...
      expect(
        resultWorkspace.workspace.documents["test-api"]!.components
      ...
      expect(
        resultWorkspace.workspace.auth["test-api"]?.secrets["api-key-auth"],
      ).toEqual({
        type: "apiKey",
    ...
    it("should transform HTTP bearer security schemes into document components and auth store", async () => {
      ...
      // Verify security scheme is in document components
      expect(
        resultWorkspace.workspace.documents["bearer-api"]!.components
      ...
      // Verify security scheme secrets are in auth store
      expect(
        resultWorkspace.workspace.auth["bearer-api"]!.secrets["bearer-auth"],
      ).toEqual({
        type: "http",
    ...
    it("should transform HTTP basic security schemes into document components and auth store", async () => {
      ...
      // Verify security scheme is in document components
      expect(
        resultWorkspace.workspace.documents["basic-auth-api"]!.components
      ...
      // Verify security scheme secrets are in auth store
      expect(
        resultWorkspace.workspace.auth["basic-auth-api"]!.secrets["basic-auth"],
      ).toEqual({
        type: "http",
    ...
    it("should transform OAuth2 security schemes into document components and auth store", async () => {
      ...
      // Verify security scheme is in document components
      const oauthScheme =
        resultWorkspace.workspace.documents["oauth-api"]!.components
          ?.securitySchemes?.["oauth2-auth"];
      if (
        oauthScheme &&
        "type" in oauthScheme &&
      ...
      // Verify security scheme secrets are in auth store
      expect(
        resultWorkspace.workspace.auth["oauth-api"]!.secrets["oauth2-auth"],
      ).toEqual({
        type: "oauth2",
        authorizationCode: {
    ...
    it("should handle multiple security schemes across multiple documents", async () => {
      ...
      expect(
        resultWorkspace.workspace.documents["api-one"]!.components
      ...
      expect(
        resultWorkspace.workspace.documents["api-two"]!.components
      ...
      expect(
        resultWorkspace.workspace.auth["api-one"]!.secrets["api-key"],
      ).toMatchObject({
        type: "apiKey",
        "x-scalar-secret-token": "key-123",
      });
      expect(
        resultWorkspace.workspace.auth["api-two"]!.secrets["bearer-token"],
      ).toMatchObject({
        type: "http",
    ...
    it("should handle collections without security schemes", async () => {
      ...
      expect(
        resultWorkspace.workspace.documents["no-auth-api"]!.components
          ?.securitySchemes,
      ).toMatchObject({});
      ...
      expect(
        Object.keys(resultWorkspace.workspace.auth["no-auth-api"]!.secrets),
      ).toHaveLength(0);
    });
  });
  ...
  describe("transformLegacyDataToWorkspace - Environments", () => {
    ...
    it("should handle multiple workspaces with different environments", async () => {
      ...
      const devEnvironments =
        result[0]?.workspace.meta["x-scalar-environments"];
      ...
      const prodEnvironments =
        result[1]?.workspace.meta["x-scalar-environments"];
  ...
  describe("transformLegacyDataToWorkspace - Cookies", () => {
    it("should transform workspace cookies into x-scalar-cookies extension", async () => {
      ...
      expect(workspaceResult.workspace.meta["x-scalar-cookies"]).toHaveLength(
        2,
      );
    ...
    it("should handle workspace with no cookies", async () => {
      ...
      expect(
        workspaceResult.workspace.meta["x-scalar-cookies"],
      ).toBeUndefined();
    });
  ...
  describe("transformLegacyDataToWorkspace - Document Meta", () => {
    ...
    describe("selectedServerUid → x-scalar-selected-server", () => {
      it("transforms selectedServerUid to x-scalar-selected-server using the server URL", async () => {
        ...
        assert(doc);
        expect(doc["x-scalar-selected-server"]).toBe(
    ...
    describe("combined document meta transformations", () => {
      ...
      it("transforms document meta across multiple collections in one workspace", async () => {
        ...
        expect(doc1["x-scalar-selected-server"]).toBe(
        ...
        expect(doc2["x-scalar-selected-server"]).toBe(
  ...
  describe("transformLegacyDataToWorkspace - Servers", () => {
    ...
    describe("servers across multiple collections and workspaces", () => {
      it("resolves different servers per collection in the same workspace", async () => {
        ...
        expect(doc1.servers).toMatchObject([
          { url: "https://api-one.example.com", description: "API One server" },
        ]);
        expect(doc1["x-scalar-selected-server"]).toBe(
          "https://api-one.example.com",
        );

        expect(doc2.servers).toMatchObject([
          { url: "https://api-two.example.com", description: "API Two server" },
        ]);
        expect(doc2["x-scalar-selected-server"]).toBe(
      ...
      it("handles collections sharing the same server record", async () => {
        ...
        expect(docBeta.servers).toMatchObject([
          {
            url: "https://shared.example.com",
            description: "Shared API server",
          },
        ...
        expect(docAlpha["x-scalar-selected-server"]).toBe(
          "https://shared.example.com",
        );
        expect(docBeta["x-scalar-selected-server"]).toBe(
          "https://shared.example.com",
        );
      });
      ...
      it("resolves servers independently across multiple workspaces", async () => {
        ...
        expect(devDoc.servers).toMatchObject([
          { url: "https://dev.example.com" },
        ]);
        expect(devDoc["x-scalar-selected-server"]).toBe(
          "https://dev.example.com",
        );

        expect(prodDoc.servers).toMatchObject([
          { url: "https://prod.example.com" },
        ]);
        expect(prodDoc["x-scalar-selected-server"]).toBe(
      ...
      it("handles one collection with servers and another without", async () => {
        ...
        const docWithout =
          resultWorkspace.workspace.documents["without-server"];

        assert(docWith);
        assert(docWithout);

        expect(docWith.servers).toMatchObject([
          { url: "https://api.example.com" },
        ]);
        expect(docWith["x-scalar-selected-server"]).toBe(
    ...
    describe("server edge cases", () => {
      ...
      it("resolves only valid servers when collection has a mix of valid and non-existent UIDs", async () => {
        ...
        expect(doc.servers).toMatchObject([
          { url: "https://valid.example.com", description: "Valid server" },
        ]);
        expect(doc["x-scalar-selected-server"]).toBe(
        ...
        expect(doc["x-tagGroups"]).toEqual([
          {
            ...
            tags: ["Dogs", "Cats"],
            "x-scalar-order": [
              "nested-tags-api/tag/dogs",
              "nested-tags-api/tag/cats",
            ],
          },
        ]);
      });
        ...
        expect(doc["x-tagGroups"]).toEqual([
          {
            ...
            tags: ["Dogs", "Cats"],
            "x-scalar-order": [
              "multi-group-api/tag/dogs",
              "multi-group-api/tag/cats",
            ],
          },
          {
            name: "Vehicles",
            tags: ["Sedans", "Trucks"],
            "x-scalar-order": [
              "multi-group-api/tag/sedans",
              "multi-group-api/tag/trucks",
            ],
          },
        ]);
      });
  ...
  describe("transformLegacyDataToWorkspace - Requests & RequestExamples", () => {
    describe("basic request transformation", () => {
      ...
      it("transforms a request with parameters", async () => {
        ...
        assert(doc);
        expect(
          getResolvedRef(doc.paths?.["/users/{id}"]?.get)?.parameters,
        ).toMatchObject([
          {
            name: "id",
            in: "path",
    ...
    describe("request examples transformation", () => {
      ...
      it("transforms an example with YAML request body", async () => {
        const example = requestExampleSchema.parse({
          ...
          parameters: {
            headers: [
              { key: "Content-Type", value: "application/yaml", enabled: true },
            ],
          },
        });
      ...
      it("transforms an example with EDN request body", async () => {
        const example = requestExampleSchema.parse({
          ...
          parameters: {
            headers: [
              { key: "Content-Type", value: "application/edn", enabled: true },
            ],
          },
        });
      ...
      it("sets x-scalar-selected-content-type for multiple examples", async () => {
        ...
        expect(
          getResolvedRef(
            getResolvedRef(doc?.paths?.["/login"]?.post)?.requestBody,
      ...
      it("does not set x-scalar-selected-content-type when there is no request body", async () => {
        ...
        const doc = result[0]?.workspace.documents["users-api"];
        expect(
          getResolvedRef(doc?.paths?.["/users"]?.get)?.requestBody,
        ).toBeUndefined();
      });
                ...
                parameters: [
                  {
                    ...
                    schema: { type: "string" },
                    examples: {
                      "Default Example": { value: "2", "x-disabled": false },
                    },
                  },
                ],
              },
      ...
      it("transforms a request with circular schema in request body", async () => {
        ...
        request.requestBody.content[
          "application/json"
        ].schema.properties.satellites.items =
                  ...
                  content: {
                    "application/json": {
                      ...
                      examples: {
                        "Create Planet": {
                          value:
                            '{"name": "Earth", "satellites": [{"name": "Moon"}]}',
                        },
                      },
                    },
                  },
    ...
    describe("circular documents", () => {
      it("migrates collection with self-referencing circular schema (tree node)", async () => {
        ...
        legacyData.records.collections[
          "collection-1"
        ].components.schemas.TreeNode.properties.children.items =
          // @ts-expect-error: Yolo
          legacyData.records.collections[
            "collection-1"
        ...
        const example = requestExampleSchema.parse({
          ...
          body: {
            ...
            raw: {
              encoding: "json",
              value:
                '{"name": "Earth", "satellites": [{"name": "Moon", "satellites": []}]}',
            },
          },
        });
  ...
  describe("document name slugification", () => {
    it("handles multiple documents with the same name by creating unique slugs", async () => {
      ...
      expect(resultWorkspace.workspace.documents["my-api"]?.info.version).toBe(
        "1.0.0",
      );
      expect(
        resultWorkspace.workspace.documents["my-api-1"]?.info.version,
      ).toBe("2.0.0");
      expect(
        resultWorkspace.workspace.documents["my-api-2"]?.info.version,
      ).toBe("3.0.0");
    });
    ...
    it("handles mix of unique and duplicate document names", async () => {
      ...
      expect(
        resultWorkspace.workspace.documents["unique-api"]?.info.version,
      ).toBe("1.0.0");
      expect(
        resultWorkspace.workspace.documents["duplicate-api"]?.info.version,
      ).toBe("1.0.0");
      expect(
        resultWorkspace.workspace.documents["duplicate-api-1"]?.info.version,
      ).toBe("2.0.0");
      expect(
        resultWorkspace.workspace.documents["another-unique-api"]?.info.version,
      ).toBe("1.0.0");
    });
    ...
    it("handles many duplicate documents with incremental naming", async () => {
      ...
      expect(
        resultWorkspace.workspace.documents["repeated-api"]?.info.version,
      ).toBe("1.0.0");
      expect(
        resultWorkspace.workspace.documents["repeated-api-1"]?.info.version,
      ).toBe("2.0.0");
      expect(
        resultWorkspace.workspace.documents["repeated-api-2"]?.info.version,
      ).toBe("3.0.0");
      expect(
        resultWorkspace.workspace.documents["repeated-api-3"]?.info.version,
      ).toBe("4.0.0");
      expect(
        resultWorkspace.workspace.documents["repeated-api-4"]?.info.version,
      ).toBe("5.0.0");
    });
    ...
    it('preserves "Drafts" special case normalization with duplicates', async () => {
      ...
      expect(resultWorkspace.workspace.documents["drafts"]?.info.version).toBe(
        "1.0.0",
      );
      expect(
        resultWorkspace.workspace.documents["drafts-1"]?.info.version,
      ).toBe("2.0.0");
    });
    ...
    it("ensures uniqueness is per workspace, not global", async () => {
      ...
      const workspace1DocNames = Object.keys(
        workspace1Result.workspace.documents,
      );
      const workspace2DocNames = Object.keys(
        workspace2Result.workspace.documents,
      );
      ...
      expect(
        workspace1Result.workspace.documents["shared-name-api"]?.info.version,
      ).toBe("1.0.0");
      expect(
        workspace1Result.workspace.documents["shared-name-api-1"]?.info.version,
      ).toBe("2.0.0");
      expect(
        workspace2Result.workspace.documents["shared-name-api"]?.info.version,
      ).toBe("3.0.0");
      expect(
        workspace2Result.workspace.documents["shared-name-api-1"]?.info.version,
      ).toBe("4.0.0");
    });
  });
});
