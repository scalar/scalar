...
describe('servers', () => {
  ...
  describe('getServersFromDocument', () => {
    ...
    it('creates fallback when all servers are invalid', () => {
      const servers: ServerObject[] = [
        { url: '' }, // This is actually valid according to the schema
        // @ts-expect-error - Testing an invalid server object
        { invalid: 'server' }, // Invalid: missing URL
      ]
    ...
    it('handles edge case with undefined servers in array', () => {
      const servers = [undefined, { url: 'https://api.example.com' }]

      const result = getServersFromDocument(servers as any)
