...
describe("getSchemas", () => {
  ...
  it("collects nested schemas with $id", () => {
    ...
    expect(result.get("https://example.com/product")).toBe(
      "definitions/product",
    );
  });
  ...
  it("uses parent $id as base for nested $anchor", () => {
    ...
    expect(result.size).toBe(2);
    expect(result.get("https://example.com/schema")).toBe("");
    expect(result.get("https://example.com/schema#user")).toBe(
      "definitions/user",
    );
  });
  ...
  it("handles deeply nested schemas", () => {
    ...
    const result = getSchemas(input);
    expect(result.size).toBe(2);
    expect(result.get("https://example.com/user")).toBe(
      "components/schemas/user",
    );
    expect(result.get("https://example.com/user#address")).toBe(
  ...
  it("handles complex nested structure with multiple schemas", () => {
    ...
    expect(result.get("https://example.com/user#user")).toBe(
      "components/schemas/user",
    );
    expect(result.get("https://example.com/user#address")).toBe(
      "components/schemas/user/properties/address",
    );
    expect(result.get("https://example.com/root#product")).toBe(
