...
describe('debounce', () => {
  ...
  it('executes the function after the default delay', () => {
    ...
    // Should execute after default delay (328ms)
    vi.advanceTimersByTime(328)
  ...
  it('executes at most once per maxWait period even with continuous calls', () => {
    const { execute } = debounce({ delay: 100, maxWait: 300 })
    const fn = vi.fn()

    // Call continuously every 50ms for 500ms
    execute('test', fn)
    vi.advanceTimersByTime(50)
    execute('test', fn)
    vi.advanceTimersByTime(50)
    ...
    // At 350ms total, the function should have executed once due to maxWait (300ms)
    expect(fn).toHaveBeenCalledTimes(1)
    ...
    // Now we are at 450ms. After the first maxWait execution at ~300ms,
    // we should not have hit another maxWait yet
    expect(fn).toHaveBeenCalledTimes(1)

    // Wait for the debounce delay to complete
    vi.advanceTimersByTime(100)

    // Now the trailing edge debounce should have executed
  ...
  it('executes immediately when maxWait is exceeded', () => {
    ...
    // Keep calling every 50ms to reset the debounce
    for (let i = 0; i < 4; i++) {
      vi.advanceTimersByTime(50)
      execute('test', fn)
    }

    // At 200ms, maxWait is hit, function should execute
    expect(fn).toHaveBeenCalledTimes(1)
  })

  it('does not interfere with normal debounce behavior when maxWait is not exceeded', () => {
    const { execute } = debounce({ delay: 100, maxWait: 500 })
    const fn = vi.fn()

    execute('test', fn)

    // Wait for normal delay without any more calls
    vi.advanceTimersByTime(100)

    // Should execute normally via debounce, not maxWait
    expect(fn).toHaveBeenCalledTimes(1)
  })

  it('handles multiple keys independently with maxWait', () => {
    const { execute } = debounce({ delay: 100, maxWait: 200 })
    const fn1 = vi.fn()
    const fn2 = vi.fn()

    // Call key1 continuously
    execute('key1', fn1)
    vi.advanceTimersByTime(50)
    execute('key1', fn1)
    vi.advanceTimersByTime(50)
    ...
    // At 100ms, neither should have executed yet
    expect(fn1).not.toHaveBeenCalled()
    expect(fn2).not.toHaveBeenCalled()
    ...
    // At 200ms total, key1 should hit maxWait
    expect(fn1).toHaveBeenCalledTimes(1)
