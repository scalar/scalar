...
describe("normalizeHttpMethod", () => {
  describe("valid HTTP methods", () => {
    it.each(HTTP_METHODS)(
      "should normalize valid HTTP method: %s",
      (method) => {
        // Test lowercase
        ...
        // Test mixed case
        expect(
          normalizeHttpMethod(method.charAt(0).toUpperCase() + method.slice(1)),
        ).toBe(method);
        expect(consoleWarnSpy).not.toHaveBeenCalled();
      },
    );
  ...
  describe("invalid inputs", () => {
    it("should return default method for undefined input", () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        "Request method is not a string. Using get as the default.",
      );
    });
