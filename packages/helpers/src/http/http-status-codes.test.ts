...
describe("HTTP Status Codes", () => {
    ...
    it("should have valid color values for all status codes", () => {
      Object.values(httpStatusCodes).forEach((status) => {
        expect(status.color).toMatch(
          /^var\(--scalar-color-(blue|green|yellow|red)\)$/,
        );
      });
    });
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle status codes with special characters in names", () => {
      const statusCodesWithSpecialChars = Object.values(httpStatusCodes).filter(
        (status) => /[^a-zA-Z0-9\s]/.test(status.name),
      );
