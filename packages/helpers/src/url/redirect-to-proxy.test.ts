...
describe("redirectToProxy", () => {
  ...
  it("handles undefined proxyUrl parameter", () => {
    expect(redirectToProxy(undefined, "https://example.com")).toBe(
      "https://example.com",
    );
  });
  ...
  it("handles malformed URLs gracefully", () => {
    expect(redirectToProxy("https://proxy.scalar.com", "not a valid url")).toBe(
      "not a valid url",
    );
  });
  ...
  it("handles proxy URLs with paths", () => {
    expect(
      redirectToProxy(
        "https://proxy.scalar.com/v1/proxy",
        "https://example.com",
      ),
    ).toBe(
...
describe("shouldUseProxy", () => {
  ...
  it("returns false for relative URLs", () => {
    ...
    expect(shouldUseProxy("https://proxy.scalar.com", "api/endpoint")).toBe(
      false,
    );
    expect(shouldUseProxy("https://proxy.scalar.com", "./api")).toBe(false);
  });
  ...
  it("returns true for local proxy URLs", () => {
    expect(
      shouldUseProxy("http://localhost:3000/proxy", "https://example.com"),
    ).toBe(true);
    expect(
      shouldUseProxy("http://127.0.0.1:8080/proxy", "https://example.com"),
    ).toBe(true);
  });

  it("returns false for localhost URLs with remote proxy", () => {
    expect(
      shouldUseProxy("https://proxy.scalar.com", "http://localhost:3000"),
    ).toBe(false);
    expect(
      shouldUseProxy("https://proxy.scalar.com", "http://127.0.0.1:8080"),
    ).toBe(false);
  });
  ...
  it("returns true for remote proxy with remote URL", () => {
    expect(
      shouldUseProxy("https://proxy.scalar.com", "https://api.example.com"),
    ).toBe(true);
  });
