...
describe("plugin-manager", () => {
  ...
  describe("executeHook", () => {
    it("executes onBeforeRequest hook with correct arguments", async () => {
      ...
      expect(onBeforeRequestHook).toHaveBeenCalledWith({
        request: mockRequest,
      });
    ...
    it("handles plugins without hooks gracefully", async () => {
      ...
      await expect(
        manager.executeHook("onBeforeRequest", { request: mockRequest }),
      ).resolves.toEqual([]);
    });
    ...
    it("returns promise that resolves when all hooks complete", async () => {
      ...
      const result = await manager.executeHook("onBeforeRequest", {
        request: mockRequest,
      });
