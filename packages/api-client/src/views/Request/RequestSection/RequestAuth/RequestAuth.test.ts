...
describe("RequestAuth.vue", () => {
  const createBaseProps = () =>
    ({
      ...
      selectedSecuritySchemeUids:
        [] as Collection["selectedSecuritySchemeUids"],
  ...
  it("calls correct mutator when selecting auth scheme", async () => {
    ...
    const bearerAuthOption = Array.from(options).find((option) =>
      option.textContent?.includes("bearerAuth"),
    );
  ...
  it("handles complex auth requirements with multiple keys", async () => {
    ...
    const options = document.querySelectorAll('li[role="option"]');
    const clientIdOption = Array.from(options).find((option) =>
      option.textContent?.includes("Client ID & Client Key"),
    );
  ...
  it("opens auth select when auth indicator is clicked", async () => {
    ...
    const requiredIndicator = wrapper
      .findAll("span")
      .find((node) => node.text().includes("Required"));
