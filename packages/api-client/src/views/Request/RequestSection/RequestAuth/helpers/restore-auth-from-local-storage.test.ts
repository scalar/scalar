...
describe("restoreAuthFromLocalStorage", () => {
  ...
  beforeEach(async () => {
    ...
    const { safeLocalStorage } = await import(
      "@scalar/helpers/object/local-storage"
    );

    vi.mocked(safeLocalStorage).mockReturnValue(
  ...
  describe("edge cases and error handling", () => {
    ...
    it("should handle mixed array and string selected schemes", () => {
      ...
      expect(mockStore.collectionMutators.edit).toHaveBeenCalledWith(
        collectionUid,
        "selectedSecuritySchemeUids",
        ["scheme-uid-1", ["scheme-uid-3", "scheme-uid-4"], "scheme-uid-2"],
      );
    });
    ...
    it("should handle empty string for selected security schemes", () => {
      ...
      mockLocalStorage.getItem
        .mockReturnValueOnce(JSON.stringify(mockAuthData))
        .mockReturnValueOnce(""); // Empty string for SELECTED_SECURITY_SCHEMES
