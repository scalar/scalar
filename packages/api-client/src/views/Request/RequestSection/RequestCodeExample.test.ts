...
describe("RequestCodeExample.vue", () => {
  ...
  it("renders correctly with default props", () => {
    ...
    expect(wrapper.findComponent({ name: "ViewLayoutCollapse" }).exists()).toBe(
      true,
    );
  });
  ...
  it("selects a different client when dropdown changes", async () => {
    ...
    expect(mockWorkspaceMutators.edit).toHaveBeenCalledWith(
      "mockWorkspaceUid",
      "selectedHttpClient",
      {
  ...
  it("filters security schemes correctly", () => {
    ...
    expect((wrapper.vm as any).selectedSecuritySchemes[0]).toEqual(
  ...
  it("includes optional selected security schemes", () => {
    ...
    // Both are selected
    mockOperation.selectedSecuritySchemeUids = [
      requiredScheme.uid,
      optionalScheme.uid,
    ];
    vi.mocked(useWorkspace).mockReturnValue({
