...
?param=value'`;
  ...
  describe("query parameters", () => {
    ...
    it("parses multiple query parameters from URL correctly", () => {
      const curlCommand =
        "curl 'http://httpbin.org/get?name=value1&age=value2'";
      const result = parseCurlCommand(curlCommand);
      expect(result.queryParameters).toStrictEqual([
    ...
    it("parses query parameters from body data correctly", () => {
      const curlCommand =
        "curl -d 'name=value1&age=value2' https://example.com";
      const result = parseCurlCommand(curlCommand);
      expect(result.queryParameters).toStrictEqual([
        { key: "name", value: "value1" },
        { key: "age", value: "value2" },
    ...
    it("handles duplicate query parameter names correctly", () => {
      const curlCommand =
        'curl "http://example.com?tag=red&tag=blue&tag=green"';
      const result = parseCurlCommand(curlCommand);
      expect(result.queryParameters).toStrictEqual([
        { key: "tag", value: "red" },
        { key: "tag", value: "blue" },
  ...
  describe("cookies", () => {
    ...
    it("handles cookies with unicode characters correctly", () => {
      const curlCommand =
        'curl -b "名称=值; cookie2=value2" http://example.com';
      const result = parseCurlCommand(curlCommand);
      expect(result.headers).toHaveProperty(
        "Cookie",
        "名称=值; cookie2=value2",
