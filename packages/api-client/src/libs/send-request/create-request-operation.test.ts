...
describe("create-request-operation", () => {
    ...
    const [error, requestOperation] = createRequestOperation(
      createRequestPayload({
        ...
        requestExamplePayload: {
          parameters: {
            path: [
              { key: "id", value: "user/123?filter=active#top", enabled: true },
            ],
          },
        },
      }),
  ...
  describe("authentication", () => {
    ...
    it("accepts a lowercase auth header", () => {
      ...
      expect(requestOperation.request.headers.get("x-api-key")).toBe(
        "test-key",
      );
    });
  });
  ...
  describe("response streaming", () => {
    it("streams the response body", async () => {
      ...
      expect(result?.response.reader).toBeInstanceOf(
  ...
  it("executes onBeforeRequest and await hook execution before sending it", async () => {
    ...
    const mockPluginManager = {
      ...
      executeHook: vi
        ...
        .mockImplementationOnce(
          async (_, { request }: { request: Request }) => {
    ...
    expect(requestOperation.request.headers.get(HEADER_MOCK_NAME)).toBe(
      HEADER_MOCK_VALUE,
    );
  });
  ...
  it("executes onResponseReceived hook when plugin manager is provided", async () => {
    ...
    expect(mockPluginManager.executeHook).toHaveBeenNthCalledWith(
      2,
      "onResponseReceived",
      {
  ...
  it("executes onResponseReceived and await hook execution before moving on", async () => {
    ...
    const mockPluginManager = {
      executeHook: vi
        ...
        .mockImplementationOnce(
          async (_, { response }: { response: Response }) => {
    ...
    expect(responseFromOperation!.headers.get(HEADER_MOCK_NAME)).toBe(
      HEADER_MOCK_VALUE,
    );
  });
  ...
  it("handles `onBeforeRequest` errors", async () => {
    ...
    await expect(requestOperation.sendRequest()).resolves.toStrictEqual([
      expect.objectContaining({
        message: "onBeforeRequest request hook failed",
      }),
      null,
    ]);
  });
