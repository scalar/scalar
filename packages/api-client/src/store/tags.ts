...
import { type Collection, type Request, type Tag, type TagPayload, tagSchema } from '@scalar/oas-utils/entities/spec'
...
export function createStoreTags(useLocalStorage: boolean) {
  const tags = reactive<Record<Tag['uid'], Tag>>({})
...
export function extendedTagDataFactory({
  ...
  const deleteTag = (tag: Tag, collectionUid: Collection['uid']) => {
    ...
    tag.children.forEach((childUid) => {
      const request = requests[childUid as Request['uid']]
      if (!request) return
