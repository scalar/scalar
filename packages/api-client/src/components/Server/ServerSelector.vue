...
<template>
  <ScalarListbox
    ...
    :teleport="`#${target}`">
    <ScalarButton
      class="text-c-1 h-6.5 w-full justify-start gap-0.75 overflow-x-auto rounded-b-lg px-3 py-1.5 text-xs font-normal whitespace-nowrap -outline-offset-1 lg:text-sm"
      variant="ghost">
  ...
  <div
    v-else
    class="text-c-1 flex h-6.5 w-full items-center gap-0.75 rounded-b-lg px-3 py-1.5 text-xs whitespace-nowrap lg:text-sm">
    <span class="sr-only">Server:</span>
    <span class="overflow-x-auto">{{ serverUrlWithoutTrailingSlash }}</span>
  </div>
</template>
