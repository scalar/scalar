...
describe("Servers", () => {
  ...
  it("emits server:add:server event when Add Server button is clicked", async () => {
    ...
    const addButton = wrapper
      .findAll("button")
      .find((b) => b.text().includes("Add Server"));
  ...
  it("emits server:delete:server event with correct index when deleting a server", async () => {
     ...
     */
    const firstDeleteButton = wrapper.find(
    ...
    expect(emitSpy).toHaveBeenCalledWith("server:delete:server", {
      index: 0,
      meta: { type: "document" },
    });
  });
  ...
  it("debounces multiple rapid server updates to the same field and only emits the last value", async () => {
     ...
     */
    const serverUpdateCalls = emitSpy.mock.calls.filter(
      (call) => call[0] === "server:update:server",
    );
