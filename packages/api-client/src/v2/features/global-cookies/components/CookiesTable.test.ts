...
const createMockCookies = (cookies: XScalarCookie[] = []): XScalarCookie[] =>
  cookies;
...
describe("CookiesTable", () => {
  ...
  it("always displays an empty row at the end for adding new cookies", () => {
     ...
     */
    const codeInputsWithCookies = wrapperWithCookies.findAllComponents({
      name: "CodeInput",
    });
     ...
     */
    const codeInputsEmpty = wrapperEmpty.findAllComponents({
      name: "CodeInput",
    });
