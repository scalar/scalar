...
describe('createApiClientModal', () => {
  ...
  it('keeps workspace document changes when modal closes and marks the document dirty', async () => {
    const workspaceStore = await setupWorkspaceStore()
    ...
    const afterClose = workspaceStore.workspace.documents[documentSlug || '']
    expect(afterClose).toBeDefined()
    expect(afterClose!.info.title).toBe('Modified Title')
    expect(afterClose!['x-scalar-is-dirty']).toBe(true)
  })
})
