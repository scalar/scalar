...
const createModalProps = async (
  ...
  const document = computed(
    () => store.workspace.documents[documentSlug.value ?? ""] ?? null,
  );
...
describe("Modal", () => {
  ...
  it("passes correct props to Operation component", async () => {
    const { props, modalState, path, method, exampleName } =
      await createModalProps();
    modalState.open = true;
  ...
  it("passes hideClientButton option to Operation component", async () => {
     ...
     */
    const operationWithHidden = wrapperWithHidden.findComponent({
      name: "OperationBlock",
    });
     ...
     */
    const operationWithShown = wrapperWithShown.findComponent({
      name: "OperationBlock",
    });
     ...
     */
    const operationWithDefault = wrapperWithDefault.findComponent({
      name: "OperationBlock",
    });
