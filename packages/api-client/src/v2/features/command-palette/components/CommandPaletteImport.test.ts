...
describe("CommandPaletteImport", () => {
  ...
  it("renders input placeholder text", () => {
    ...
    expect(input.props("placeholder")).toBe(
      "OpenAPI/Swagger/Postman URL or cURL",
    );
  });
  ...
  it("clears content when clear button is clicked", async () => {
    ...
    const clearButton = wrapper
      .findAll("button")
      .find((btn) => btn.text().includes("Clear"));
    await clearButton?.trigger("click");
    await nextTick();

    /** After clearing, should go back to input mode */
    expect(wrapper.findComponent({ name: "CommandActionInput" }).exists()).toBe(
      true,
    );
  });
  ...
  it("shows different tooltip content for URL vs non-URL input", async () => {
    ...
    expect(tooltip.props("content")).toContain(
      "only available for URL imports",
    );
