...
describe("get-operation-from-curl", () => {
  ...
  it("parses POST request with JSON body", () => {
    const curl =
      'curl -X POST http://example.com/api/users -d \'{"name":"John","age":30}\'';
    const result = getOperationFromCurl(curl);
  ...
  it("parses request with query parameters", () => {
    ...
    const limitParam = getResolvedRef(
      result.operation.parameters?.find(
        (p) => getResolvedRef(p)?.name === "limit",
      ),
    );
  ...
  it("parses request with headers", () => {
    ...
    const authParam = getResolvedRef(
      result.operation.parameters?.find(
        (p) => getResolvedRef(p)?.name === "Authorization",
      ),
    );
    ...
    const contentTypeParam = getResolvedRef(
      result.operation.parameters?.find(
        (p) => getResolvedRef(p)?.name === "Content-Type",
      ),
    );
  ...
  it("parses request with both query parameters and headers", () => {
    const curl =
      'curl http://example.com/api/users?limit=10 -H "Authorization: Bearer token"';
    const result = getOperationFromCurl(curl);

    expect(result.operation.parameters).toHaveLength(2);

    const queryParam = getResolvedRef(
      result.operation.parameters?.find(
        (p) => getResolvedRef(p)?.in === "query",
      ),
    );
    expect(queryParam?.name).toBe("limit");

    const headerParam = getResolvedRef(
      result.operation.parameters?.find(
        (p) => getResolvedRef(p)?.in === "header",
      ),
    );
  ...
  it("parses request with custom example key", () => {
    ...
    const customExample = getResolvedRef(
      limitParam.examples?.["custom-example"],
    );
  ...
  it("detects JSON content type from JSON body with explicit header", () => {
    const curl =
      'curl -X POST http://example.com/api/users -H "Content-Type: application/json" -d \'{"name":"John"}\'';
    const result = getOperationFromCurl(curl);
  ...
  it("auto-detects JSON content type from JSON body without header", () => {
    const curl =
      'curl -X POST http://example.com/api/users -d \'{"name":"John"}\'';
    const result = getOperationFromCurl(curl);
  ...
  it("auto-detects JSON content type from JSON array without header", () => {
    const curl =
      'curl -X POST http://example.com/api/users -d \'[{"name":"John"},{"name":"Jane"}]\'';
    const result = getOperationFromCurl(curl);
  ...
  it("infers boolean type for boolean values", () => {
    const curl =
      "curl -X POST http://example.com/api/users -d '{\"active\":true}'";
    const result = getOperationFromCurl(curl);
  ...
  it("infers null type for null values", () => {
    const curl =
      "curl -X POST http://example.com/api/users -d '{\"deletedAt\":null}'";
    const result = getOperationFromCurl(curl);
  ...
  it("infers array type for array values", () => {
    const curl =
      'curl -X POST http://example.com/api/users -d \'{"tags":["tag1","tag2"]}\'';
    const result = getOperationFromCurl(curl);
  ...
  it("infers object type for nested object values", () => {
    const curl =
      'curl -X POST http://example.com/api/users -d \'{"address":{"city":"NYC"}}\'';
    const result = getOperationFromCurl(curl);
  ...
  it("handles form-encoded data with URL encoding", () => {
    const curl =
      'curl -X POST http://example.com/api/users -d "name=John%20Doe&email=john%40example.com"';
    const result = getOperationFromCurl(curl);

    const requestBody = getResolvedRef(result.operation.requestBody);
    const curlExample = getResolvedRef(
      requestBody?.content["application/x-www-form-urlencoded"]?.examples?.curl,
    );
  ...
  it("handles form-encoded data with missing values", () => {
    const curl =
      'curl -X POST http://example.com/api/users -d "name=John&age="';
    const result = getOperationFromCurl(curl);
  ...
  it("handles form-encoded data with malformed pairs", () => {
    const curl =
      'curl -X POST http://example.com/api/users -d "name=John&invalidpair&age=30"';
    const result = getOperationFromCurl(curl);
  ...
  it("preserves example values in request body", () => {
    ...
    const requestBody = getResolvedRef(result.operation.requestBody);
    const curlExample = getResolvedRef(
      requestBody?.content["application/json"]?.examples?.curl,
    );
  ...
  it("handles multiple headers with same example key", () => {
    const curl =
      'curl http://example.com/api/users -H "X-API-Key: key1" -H "X-Request-ID: req123"';
    const result = getOperationFromCurl(curl);

    const apiKeyParam = getResolvedRef(
      result.operation.parameters?.find(
        (p) => getResolvedRef(p)?.name === "X-API-Key",
      ),
    );
    ...
    const requestIdParam = getResolvedRef(
      result.operation.parameters?.find(
        (p) => getResolvedRef(p)?.name === "X-Request-ID",
      ),
    );
  ...
  it("handles PUT request", () => {
    const curl =
      'curl -X PUT http://example.com/api/users/1 -d \'{"name":"Jane"}\'';
    const result = getOperationFromCurl(curl);
  ...
  it("handles PATCH request", () => {
    const curl =
      'curl -X PATCH http://example.com/api/users/1 -d \'{"name":"Jane"}\'';
    const result = getOperationFromCurl(curl);
  ...
  it("handles numeric query parameter values", () => {
    ...
    const ageParam = getResolvedRef(
      result.operation.parameters?.find(
        (p) => getResolvedRef(p)?.name === "age",
      ),
    );
  ...
  it("handles boolean-like query parameter values", () => {
    ...
    const activeParam = getResolvedRef(
      result.operation.parameters?.find(
        (p) => getResolvedRef(p)?.name === "active",
      ),
    );
  ...
  it("handles deeply nested JSON structures", () => {
    const curl =
      'curl -X POST http://example.com/api/users -d \'{"level1":{"level2":{"level3":{"value":"deep"}}}}\'';
    const result = getOperationFromCurl(curl);
  ...
  it("handles mixed type arrays in JSON", () => {
    const curl =
      'curl -X POST http://example.com/api/users -d \'{"mixed":[1,"two",true,null]}\'';
    const result = getOperationFromCurl(curl);
  ...
  it("handles special characters in header values", () => {
    const curl =
      'curl http://example.com/api/users -H "X-Custom: value!@#$%^&*()"';
    const result = getOperationFromCurl(curl);

    const customParam = getResolvedRef(
      result.operation.parameters?.find(
        (p) => getResolvedRef(p)?.name === "X-Custom",
      ),
    );
  ...
  it("handles unicode characters in JSON body", () => {
    const curl =
      'curl -X POST http://example.com/api/users -d \'{"name":"用户","emoji":"🚀"}\'';
    const result = getOperationFromCurl(curl);
  ...
  it("handles empty string values in JSON", () => {
    const curl =
      'curl -X POST http://example.com/api/users -d \'{"name":"","age":30}\'';
    const result = getOperationFromCurl(curl);
  ...
  it("handles false values in JSON", () => {
    const curl =
      "curl -X POST http://example.com/api/users -d '{\"active\":false}'";
    const result = getOperationFromCurl(curl);
  ...
  it("handles multiple query parameters with same name", () => {
    ...
    const tagParams = result.operation.parameters?.filter(
      (p) => getResolvedRef(p)?.name === "tag",
    );
