...
describe("useCommandPaletteState", () => {
  ...
  it("filters commands by name case-insensitively", async () => {
    ...
    const allCommands = state.filteredCommands.value.flatMap(
      (group) => group.commands,
    );
    expect(allCommands).toHaveLength(1);
    expect(
      allCommands.some((cmd) => cmd.name.toLowerCase().includes("import")),
    ).toBe(true);
  });
  ...
  it("filters commands with lowercase query", async () => {
    ...
    expect(allCommands).toHaveLength(1);
    expect(
      allCommands.every((cmd) => cmd.name.toLowerCase().includes("import")),
    ).toBe(true);
  });
  ...
  it("excludes empty groups after filtering", async () => {
    ...
    const groupsWithCommands = state.filteredCommands.value.filter(
  ...
  it("returns empty array when no commands match filter", async () => {
    ...
    const totalCommands = state.filteredCommands.value.reduce(
  ...
  it("handles partial name matches", async () => {
    ...
    expect(
      allCommands.some((cmd) => cmd.id === "create-openapi-document"),
    ).toBe(true);
  });
  ...
  it("reacts to changes in reactive actions array", async () => {
    ...
    expect(state.filteredCommands.value[0]?.commands[0]?.name).toBe(
      "Test Action 1",
    );
    ...
    expect(state.filteredCommands.value[0]?.commands[1]?.name).toBe(
      "Test Action 2",
    );
  });
  ...
  it("reacts to changes in reactive routes array", async () => {
    ...
    expect(state.filteredCommands.value[1]?.commands[0]?.name).toBe(
      "Test Route 1",
    );
    ...
    expect(state.filteredCommands.value[1]?.commands[1]?.name).toBe(
      "Test Route 2",
    );
  });
  ...
  it("reacts when removing items from reactive actions", async () => {
    ...
    /** Remove an action */
    reactiveActions.value = reactiveActions.value.filter(
      (a) => a.id !== "action-2",
    );
    ...
    expect(state.filteredCommands.value[0]?.commands).toHaveLength(2);
    expect(
      state.filteredCommands.value[0]?.commands.some(
        (a) => a.id === "action-2",
      ),
    ).toBe(false);
  });
  ...
  it("handles reactive actions with hidden property changes", async () => {
    ...
    expect(state.filteredCommands.value[0]?.commands).toHaveLength(1);
    expect(state.filteredCommands.value[0]?.commands[0]?.id).toBe(
      "visible-action",
    );

    /** Change hidden action to visible */
    reactiveActions.value = reactiveActions.value.map((a) =>
      a.id === "hidden-action" ? { ...a, hidden: false } : a,
    );
