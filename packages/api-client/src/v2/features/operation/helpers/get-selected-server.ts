...
export const getSelectedServer = (document: WorkspaceDocument | null) => {
  ...
  if (typeof document?.['x-scalar-selected-server'] === 'undefined' && document?.servers?.length) {
    return document.servers[0]!
  }

  // Return the server matching the x-scalar-selected-server URL
  return document?.servers?.find(({ url }) => url === document?.['x-scalar-selected-server']) ?? null
}
