...
describe("get-yaml-node-range-from-path", () => {
  ...
  it("returns offsets for a nested scalar value", () => {
    ...
    const range = getYamlNodeRangeFromPath(yamlText, [
      "paths",
      "/pets",
      "get",
      "summary",
    ...
    expect(yamlText.slice(range!.startOffset, range!.startOffset + 7)).toBe(
      "summary",
    );
  ...
  it("returns offsets for values inside sequences", () => {
    ...
    expect(range).not.toBeNull();
    expect(yamlText.slice(range!.startOffset, range!.startOffset + 3)).toBe(
      "url",
    );
    expect(range!.endOffset).toBeGreaterThan(range!.startOffset);
  });
