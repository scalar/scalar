...
describe("RequestBodyForm", () => {
  ...
  it("initializes localFormBodyRows from example prop and syncs on changes", async () => {
    ...
    expect(requestTable.props("data")).toEqual([
      { name: "field3", value: "value3", isDisabled: false },
    ]);
  ...
  it("handles upsert and delete row operations with correct event emissions", async () => {
    ...
    await requestTable.vm.$emit("upsertRow", 10, {
      name: "field2",
      value: "value2",
    });
    await nextTick();
    ...
    if (Array.isArray(lastAddEvent)) {
      ...
      expect(
        lastAddEvent.some(
          (row) => row.name === "field2" && row.value === "value2",
        ),
      ).toBe(true);
    }

    // Test update row (upsert with existing index)
    await requestTable.vm.$emit("upsertRow", 0, {
      name: "updatedField1",
      value: "updatedValue1",
    });
    await nextTick();
    ...
    const lastDisabledEvent =
      disabledUpdateEvents?.[disabledUpdateEvents.length - 1]?.[0];
  ...
  it("handles file upload for existing and new rows correctly", async () => {
    ...
    if (fileDialogOnChange) {
      const mockFile = new File(["content"], "uploaded.txt", {
        type: "text/plain",
      });
      const fileList = {
        0: mockFile,
    ...
    if (fileDialogOnChange) {
      const mockFile = new File(["new content"], "newfile.pdf", {
  ...
  it("renders correct RequestTable variant based on selectedContentType with proper props", async () => {
    ...
    expect(multipartTable.props("data")).toEqual([
      { name: "field1", value: "value1", isDisabled: false },
    ]);
    ...
    expect(urlEncodedTable.props("data")).toEqual([
      { name: "field1", value: "value1", isDisabled: false },
    ]);
    ...
    const lastRemoveFileEvent =
      removeFileEvents?.[removeFileEvents.length - 1]?.[0];
  ...
  it("defaults isDisabled to false when adding a new row", async () => {
    ...
    if (Array.isArray(lastEvent)) {
      ...
      const newRow = lastEvent.find(
        (row) => row.name === "newField" && row.value === "newValue",
      );
      expect(newRow).toBeDefined();
  ...
  it("defaults isDisabled to false when adding a new row even if isDisabled is explicitly set to true in payload", async () => {
    ...
    await requestTable.vm.$emit("upsertRow", 5, {
      name: "newField",
      value: "newValue",
      isDisabled: true,
    });
