...
describe("validateParameter", () => {
  ...
  it("returns true when schema has no type", () => {
    expect(validateParameter({ __scalar_: "" }, "some value")).toEqual({
      ok: true,
    });
  });
  ...
  it("correctly validate a number type #2", () => {
    expect(
      validateParameter({ type: "number", minimum: 10, maximum: 100 }, "200"),
    ).toEqual({
      ok: false,
  ...
  it("correctly validate a number type #3", () => {
    expect(
      validateParameter({ type: "number", minimum: 10, maximum: 100 }, "0"),
    ).toEqual({
      ok: false,
