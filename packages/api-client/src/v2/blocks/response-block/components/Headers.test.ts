...
describe("Headers", () => {
  ...
  describe("known headers with links", () => {
    ...
    it("renders plain text for unknown headers", async () => {
      const headers: Header[] = [
        { name: "X-Custom-Unknown-Header", value: "custom value" },
      ];
  ...
  describe("accessibility", () => {
    it("includes screen reader only header labels", async () => {
      ...
      const headerLabelsElement = srOnly.find((el) =>
        el.text().includes("Header Key"),
      );
  ...
  describe("edge cases", () => {
    it("handles headers with colons in values", async () => {
      const headers: Header[] = [
        { name: "Link", value: '<https://example.com>; rel="canonical"' },
      ];
    ...
    it("handles headers with very long names", async () => {
      const longName =
        "X-Very-Long-Custom-Header-Name-That-Is-Unusually-Extended";
