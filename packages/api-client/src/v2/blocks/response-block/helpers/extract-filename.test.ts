...
describe("extractFilename", () => {
  describe("UTF-8 encoded filenames", () => {
    ...
    it("extracts UTF-8 encoded filename with unicode characters", () => {
      const result = extractFilename(
        "attachment; filename*=UTF-8''%E2%9C%93%20document.txt",
      );
    ...
    it("extracts UTF-8 encoded filename with multiple encoded characters", () => {
      const result = extractFilename(
        "attachment; filename*=UTF-8''%E4%B8%AD%E6%96%87%E6%96%87%E4%BB%B6.doc",
      );
    ...
    it("extracts UTF-8 encoded filename followed by other parameters", () => {
      const result = extractFilename(
        "attachment; filename*=UTF-8''report.pdf; size=1024",
      );
    ...
    it("prioritizes UTF-8 encoded filename over regular filename", () => {
      const result = extractFilename(
        "attachment; filename=\"fallback.txt\"; filename*=UTF-8''priority.txt",
      );
  ...
  describe("regular filename format", () => {
    ...
    it("extracts filename with spaces in quotes", () => {
      const result = extractFilename(
        'attachment; filename="my important file.docx"',
      );
    ...
    it("extracts filename with special characters in quotes", () => {
      const result = extractFilename(
        'attachment; filename="file-name_2024.txt"',
      );
  ...
  describe("URI decoding edge cases", () => {
    ...
    it("handles mixed encoded and unencoded characters", () => {
      const result = extractFilename(
        'attachment; filename="my%20file%20(1).txt"',
      );
  ...
  describe("filenames with extensions", () => {
    it("extracts filename with common extensions", () => {
      ...
      expect(extractFilename('attachment; filename="sheet.xlsx"')).toBe(
        "sheet.xlsx",
      );
      expect(extractFilename('attachment; filename="image.jpg"')).toBe(
        "image.jpg",
      );
      expect(extractFilename('attachment; filename="video.mp4"')).toBe(
        "video.mp4",
      );
      expect(extractFilename('attachment; filename="archive.zip"')).toBe(
        "archive.zip",
      );
    });
    ...
    it("extracts filename with multiple dots", () => {
      const result = extractFilename(
        'attachment; filename="backup.2024.01.15.tar.gz"',
      );
  ...
  describe("real-world examples", () => {
    ...
    it("handles content disposition with creation date", () => {
      const result = extractFilename(
        'attachment; filename="export.csv"; creation-date="Mon, 01 Jan 2024 00:00:00 GMT"',
      );
