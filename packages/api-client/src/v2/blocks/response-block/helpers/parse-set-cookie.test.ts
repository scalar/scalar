...
describe("parseSetCookie", () => {
  describe("valid cookie parsing", () => {
    ...
    it("parses cookie with multiple attributes", () => {
      const result = parseSetCookie(
        "token=xyz789; Path=/; Secure; HttpOnly; SameSite=Strict",
      );
    ...
    it("parses cookie with Expires attribute", () => {
      const result = parseSetCookie(
        "session=abc; Expires=Wed, 21 Oct 2025 07:28:00 GMT",
      );
  ...
  describe("real-world scenarios", () => {
    it("parses session cookie from Express.js", () => {
      const result = parseSetCookie(
        "connect.sid=s%3A1234567890.abcdefghijklmnop; Path=/; HttpOnly",
      );
    ...
    it("parses JWT cookie", () => {
      const result = parseSetCookie(
        "token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9; Secure; HttpOnly; SameSite=Strict",
      );

      expect(result).toEqual({
        name: "token",
        value:
          "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9; Secure; HttpOnly; SameSite=Strict",
      });
    });

    it("parses authentication cookie with expiry", () => {
      const result = parseSetCookie(
        "auth=abc123; Expires=Wed, 21 Oct 2025 07:28:00 GMT; Path=/; Secure; HttpOnly",
      );

      expect(result).toEqual({
        name: "auth",
        value:
          "abc123; Expires=Wed, 21 Oct 2025 07:28:00 GMT; Path=/; Secure; HttpOnly",
      });
    });
    ...
    it("parses tracking cookie", () => {
      ...
      expect(result).toEqual({
        name: "_ga",
        value:
          "GA1.2.1234567890.1234567890; Expires=Thu, 31 Dec 2026 23:59:59 GMT; Path=/; Domain=.example.com",
      });
    });

    it("parses CSRF token cookie", () => {
      const result = parseSetCookie(
        "XSRF-TOKEN=abc-123-def-456; Path=/; SameSite=Strict",
      );
    ...
    it("parses cookie being deleted (Max-Age=0)", () => {
      const result = parseSetCookie(
        "deleted=; Path=/; Max-Age=0; Expires=Thu, 01 Jan 1970 00:00:00 GMT",
      );
