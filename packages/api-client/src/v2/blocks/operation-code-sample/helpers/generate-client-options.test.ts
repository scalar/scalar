...
describe("generateClientOptions", () => {
  ...
  describe("with subset of clients allowed", () => {
    it("includes only allowed clients", () => {
      ...
      expect(jsGroup?.options.map((option) => option.id)).toEqual([
        "js/fetch",
        "js/axios",
      ]);
      ...
      expect(nodeGroup?.options.map((option) => option.id)).toEqual([
        "node/fetch",
      ]);
  ...
  describe("curl special handling", () => {
    it('sets lang to "curl" for curl client', () => {
      ...
      const curlOption = shellGroup?.options.find(
        (option) => option.id === "shell/curl",
      );
    ...
    it("sets lang to group key for non-curl clients", () => {
      ...
      const jsGroup = result.find((group) => group.label === "JavaScript");
      const fetchOption = jsGroup?.options.find(
        (option) => option.id === "js/fetch",
      );
    ...
    it("sets lang to group key for other shell clients", () => {
      ...
      const shellGroup = result.find((group) => group.label === "Shell");
      const httpieOption = shellGroup?.options.find(
        (option) => option.id === "shell/httpie",
      );
      const wgetOption = shellGroup?.options.find(
        (option) => option.id === "shell/wget",
      );
  ...
  describe("option structure", () => {
    ...
    it("formats titles correctly", () => {
      ...
      expect(allOptions.find((opt) => opt.id === "js/fetch")?.title).toBe(
        "JavaScript Fetch",
      );
      expect(
        allOptions.find((opt) => opt.id === "python/requests")?.title,
      ).toBe("Python Requests");
      expect(allOptions.find((opt) => opt.id === "node/axios")?.title).toBe(
        "Node.js Axios",
      );
    });
  });
  ...
  describe("multiple clients from same target", () => {
    it("groups multiple JavaScript clients together", () => {
      ...
      expect(jsGroup!.options.map((opt) => opt.id)).toEqual([
        "js/fetch",
        "js/axios",
        "js/ofetch",
      ]);
  ...
  describe("edge cases", () => {
    ...
    it("maintains consistent ordering", () => {
      const result1 = generateClientOptions([
        "python/requests",
        "js/fetch",
        "node/axios",
      ]);
      const result2 = generateClientOptions([
        "node/axios",
        "js/fetch",
        "python/requests",
      ]);
