...
describe('RequestExample', () => {
  ...
  describe('Show Schema toggle', () => {
    const operationWithSchema: OperationObject = {
      summary: 'Test operation with schema',
      requestBody: {
        content: {
          'application/json': {
            schema: coerceValue(SchemaObjectSchema, {
    ...
    it('renders the Show Schema checkbox when schema is present', () => {
      const wrapper = mount(RequestExample, {
        props: {
          ...defaultProps,
      ...
      const checkbox = wrapper.find('.scalar-card-checkbox-input')
      expect(checkbox.exists()).toBe(true)
    })

    it('does not render the Show Schema checkbox when no schema is present', () => {
      const wrapper = mount(RequestExample, {
        props: defaultProps,
      })
    ...
    it('toggles between code snippet and schema view', async () => {
      const wrapper = mount(RequestExample, {
        props: {
          ...defaultProps,
          operation: operationWithSchema,
      ...
      // Initially shows the code snippet
      const codeBlocks = wrapper.findAllComponents({ name: 'ScalarCodeBlock' })
      ...
      // Should now show schema content
      const schemaCodeBlock = wrapper.findAllComponents({ name: 'ScalarCodeBlock' })
      expect(schemaCodeBlock.length).toBeGreaterThan(0)
    })
  })
})
