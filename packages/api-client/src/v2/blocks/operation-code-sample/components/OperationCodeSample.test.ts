...
describe("RequestExample", () => {
  ...
  describe("Component Rendering", () => {
    it("renders the component with basic props", () => {
      ...
      expect(
        wrapper.findComponent({ name: "HttpMethod" }).props("method"),
      ).toBe("get");
  ...
  describe("Configuration Options", () => {
    it("hides client selector when configured", () => {
      ...
      expect(wrapper.find('[data-testid="client-picker"]').exists()).toBe(
        false,
      );
    });
  ...
  describe("Webhook Copy as cURL", () => {
    ...
    it("renders copy button for webhook payloads", () => {
      ...
      const copyComponent = wrapper.findComponent({
        name: "ScalarCodeBlockCopy",
      });
                ...
                "application/xml": {
                  examples: {
                    xmlExample: {
                      ...
                      value:
                        '<event type="user.created"><user id="123" name="John"/></event>',
                    },
                  },
                },
              },
    ...
    it("copies webhook payload with security credentials hidden", async () => {
      ...
      const codeBlock = wrapper.findComponent({ name: "ScalarCodeBlock" });
      expect(codeBlock.props("hideCredentials")).toContain(
        "secret-api-key-123",
      );
    ...
    it("handles webhook copy with different HTTP methods", async () => {
      ...
      for (const method of methods) {
        ...
        const copyButton = wrapper.findComponent({
          name: "ScalarCodeBlockCopy",
        });
        expect(copyButton.exists()).toBe(true);
