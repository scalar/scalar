...
vi.mock(
  "@/v2/blocks/scalar-auth-selector-block/helpers/fetch-openid-connect-discovery",
  () => ({
    fetchOpenIDConnectDiscovery: (...args: unknown[]) =>
      mockFetchOpenIDConnectDiscovery(...args),
  }),
);

vi.mock(
  "@/v2/blocks/scalar-auth-selector-block/helpers/openid-discovery-to-flows",
  () => ({
    openIDDiscoveryToFlows: (...args: unknown[]) =>
      mockOpenIDDiscoveryToFlows(...args),
  }),
);
...
describe("OpenIDConnect", () => {
    ...
    mount(OpenIDConnect, {
      ...
      props: {
        ...
        scheme: {
          type: "openIdConnect",
          openIdConnectUrl:
            custom.openIdConnectUrl ?? "https://issuer.example.com",
        },
      },
    });
  ...
  it("shows an error toast when discovery fetch fails", async () => {
    ...
    const emitted = vi.fn();
    eventBus.on("auth:update:security-scheme-secrets", emitted);
    mockFetchOpenIDConnectDiscovery.mockResolvedValue([
      new Error("Failed to load discovery"),
      null,
    ]);
