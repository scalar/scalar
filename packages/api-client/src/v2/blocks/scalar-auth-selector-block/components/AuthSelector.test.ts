...
describe("AuthSelector", () => {
  ...
  const mountWithProps = (
    ...
    const securityRequirements = custom.securityRequirements ?? [
      { BearerAuth: [] },
    ];
  ...
  describe("rendering", () => {
    ...
    it("renders combobox for auth selection", () => {
      ...
      const combobox = wrapper.findComponent({
        name: "ScalarComboboxMultiselect",
      });
  ...
  describe("selected scheme options computation", () => {
    ...
    it("handles multiple selected schemes", () => {
      const wrapper = mountWithProps({
        selectedSecurity: {
          selectedIndex: 0,
          selectedSchemes: [{ BearerAuth: [] }, { ApiKeyAuth: [] }],
        },
        securitySchemes: baseSecuritySchemes,
      });
    ...
    it("handles complex auth schemes", () => {
      ...
      expect(vm.selectedSchemeOptions[0]?.label).toContain(
    ...
    it("filters out undefined schemes", () => {
      const wrapper = mountWithProps({
        selectedSecurity: {
          selectedIndex: 0,
          selectedSchemes: [{ BearerAuth: [] }, { NonExistentAuth: [] }],
        },
        securitySchemes: baseSecuritySchemes,
      });
