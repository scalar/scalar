...
describe("OAuthScopesInput", () => {
  ...
  it("selects and deselects a scope via row click", async () => {
    ...
    const firstEmit = wrapper
      .emitted("update:selectedScopes")
      ?.at(-1)?.[0] as any;
  ...
  it("handles empty scopes gracefully", async () => {
    ...
    expect(
      deselectAllBtn,
      "Deselect All button should exist when there are 0/0 scopes",
    ).toBeTruthy();
    await deselectAllBtn!.trigger("click");
