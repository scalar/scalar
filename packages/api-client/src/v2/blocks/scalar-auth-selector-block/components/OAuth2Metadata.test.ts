...
describe('OAuth2Metadata', () => {
  ...
  it.each([false, true])('checks the server URL rather than proxy identity (changed URL: %s)', async (changedUrl) => {
    const eventBus = createWorkspaceEventBus()
    const update = vi.fn()
    eventBus.on('auth:update:security-scheme', update)
    vi.useFakeTimers()
    ...
    await wrapper.setProps({
      scheme: {
        type: 'oauth2',
        flows: {},
        oauth2MetadataUrl: changedUrl ? 'https://other.example/metadata' : 'https://example.com/metadata',
      },
    })
    resolveResponse(
      Response.json({ token_endpoint: 'https://example.com/token', grant_types_supported: ['client_credentials'] }),
