...
import {
  type ConfigAuthScheme,
  extractSecuritySchemeSecrets,
} from "./extract-security-scheme-secrets";
import type {
  ApiKeyObjectSecret,
  HttpObjectSecret,
  OAuth2ObjectSecret,
...
describe("extractSecuritySchemeSecrets", () => {
  ...
  describe("oauth2 security scheme - authorizationCode flow", () => {
    ...
    it("handles authorizationCode flow with PKCE enabled", () => {
      ...
      expect(
        (result as OAuth2ObjectSecret).flows.authorizationCode?.["x-usePkce"],
      ).toBe("SHA-256");
    });
  ...
  describe("oauth2 security scheme - multiple flows", () => {
    it("handles oauth2 with multiple flows", () => {
      ...
      authStore.setAuthSecrets(documentSlug, schemeName, {
        ...
        implicit: {
          ...
          "x-scalar-secret-redirect-uri":
            "https://implicit.example.com/callback",
    ...
    it("handles oauth2 with all four flows", () => {
      ...
      expect(
        (result as OAuth2ObjectSecret).flows.clientCredentials,
      ).toBeDefined();
      expect(
        (result as OAuth2ObjectSecret).flows.authorizationCode,
      ).toBeDefined();
    });
    ...
    it("returns openIdConnect scheme with multiple discovered flows from auth store", () => {
      ...
      authStore.setAuthSecrets(documentSlug, schemeName, {
        ...
        authorizationCode: {
          ...
          "x-scalar-secret-redirect-uri":
            "https://app.example.com/auth-code/callback",
          "x-scalar-secret-token": "auth-code-token",
        },
      });
  ...
  describe("different document and scheme names", () => {
    it("handles different document slugs correctly", () => {
      ...
      const result1 = extractSecuritySchemeSecrets(
        scheme,
        authStore,
        schemeName,
        "document-1",
      );
      const result2 = extractSecuritySchemeSecrets(
        scheme,
        authStore,
        schemeName,
        "document-2",
      ...
      expect((result1 as ApiKeyObjectSecret)["x-scalar-secret-token"]).toBe(
        "token-1",
      );
      expect((result2 as ApiKeyObjectSecret)["x-scalar-secret-token"]).toBe(
        "token-2",
      );
    });
