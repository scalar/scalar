...
const secretAuth = encode(
  `${baseFlow["x-scalar-secret-client-id"]}:${clientSecret}`,
);
...
describe("oauth", () => {
  ...
  describe("Authorization Code Grant", () => {
      ...
      expect(window.open).toHaveBeenCalledWith(
        new URL(
          `${scheme.authorizationCode.authorizationUrl}?${new URLSearchParams({
            response_type: "code",
            redirect_uri:
              scheme.authorizationCode["x-scalar-secret-redirect-uri"],
      ...
      expect(window.open).toHaveBeenCalledWith(
        new URL(
          `${flows.authorizationCode.authorizationUrl}?${new URLSearchParams({
            ...
            redirect_uri:
              flows.authorizationCode["x-scalar-secret-redirect-uri"],
            prompt: "login",
            audience: "scalar",
    ...
    it("should handle authorization code flow with body-only credentials location", async () => {
      ...
      expectedParams.set(
        "client_id",
        flows.authorizationCode["x-scalar-secret-client-id"],
      );
      expectedParams.set(
        "client_secret",
        flows.authorizationCode["x-scalar-secret-client-secret"],
      );
      expectedParams.set(
        "redirect_uri",
        flows.authorizationCode["x-scalar-secret-redirect-uri"],
  ...
  describe("Client Credentials Grant", () => {
    ...
    it("should use custom token name when x-tokenName is specified", async () => {
      ...
      global.fetch = vi.fn().mockResolvedValueOnce({
        json: () =>
          Promise.resolve({ custom_access_token: "custom_token_123" }),
      });
    ...
    it("should handle client credentials flow with body-only credentials location", async () => {
      ...
      expect(body.toString()).toBe(
        new URLSearchParams({
          ...
          client_secret:
            flows.clientCredentials["x-scalar-secret-client-secret"],
          grant_type: "client_credentials",
        }).toString(),
      );
    });
  ...
  describe("Password Grant", () => {
    ...
    it("should resolve relative token URLs against relative server URLs", async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        "https://example.com/partners/auth/token",
        {
          method: "POST",
          body: new URLSearchParams({
            scope: scope.join(" "),
  ...
  describe("Server variable interpolation", () => {
    it("token URL resolved correctly when server URL contains {variable} — client credentials flow", async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        "https://prod.api.example.com/oauth/token",
        expect.any(Object),
      );
    });
    ...
    it("token URL resolved via environment variables — client credentials flow", async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        "https://void.scalar.com/oauth/token",
        expect.any(Object),
      );
    });
  });
