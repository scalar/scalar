...
describe("security-scheme", () => {
  ...
  describe("getSecuritySchemeOptions", () => {
    ...
    it("should return grouped options", () => {
      ...
      const result = getSecuritySchemeOptions(
        security,
        securitySchemes,
        [],
        true,
      ...
      expect(
        groups[1].options.some((opt) => opt.id === "48cc5a8ff1d2df93"),
      ).toBe(true);
      expect(
        groups[1].options.some((opt) => opt.id === "8da8c10db72dcac3"),
      ).toBe(true);

      // Check add new authentication options
      expect(groups[2].options.length).toBeGreaterThan(0);
      expect(groups[2].options.every((opt) => opt.isDeletable === false)).toBe(
        true,
      );
    });
    ...
    it("should include all auth options in add new section", () => {
      ...
      expect(addNewOptions.some((opt) => opt.id === "oauth2Implicit")).toBe(
        true,
      );
      expect(addNewOptions.some((opt) => opt.id === "oauth2Password")).toBe(
        true,
      );
      expect(
        addNewOptions.some((opt) => opt.id === "oauth2ClientCredentials"),
      ).toBe(true);
      expect(
        addNewOptions.some((opt) => opt.id === "oauth2AuthorizationFlow"),
      ).toBe(true);

      // All should be non-deletable
      expect(addNewOptions.every((opt) => opt.isDeletable === false)).toBe(
        true,
      );
    });
    ...
    it("should handle schemes with undefined resolved references", () => {
      ...
      expect(availableOptions.some((opt) => opt.id === "undefinedScheme")).toBe(
        false,
      );
    });
