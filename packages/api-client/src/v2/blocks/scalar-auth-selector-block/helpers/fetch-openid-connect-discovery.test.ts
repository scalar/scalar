...
const mockRedirectToProxy = vi.fn((proxyUrl: string, targetUrl: string) =>
  proxyUrl
    ? `${proxyUrl}?scalar_url=${encodeURIComponent(targetUrl)}`
    : targetUrl,
);

vi.mock("@scalar/helpers/url/redirect-to-proxy", () => ({
  redirectToProxy: (proxyUrl: string, targetUrl: string) =>
    mockRedirectToProxy(proxyUrl, targetUrl),
}));
...
describe("fetch-openid-connect-discovery", () => {
  ...
  it("returns an error when the response is not ok", async () => {
    ...
    expect(error?.message).toContain(
      "Failed to fetch OpenID Connect discovery document: 404 Not Found",
    );
    expect(data).toBeNull();
  });
