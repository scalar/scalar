...
import {
  ...
  getCookieHeader,
} from "./build-request-cookie-header";
...
describe("buildRequestCookieHeader", () => {
  describe("basic functionality", () => {
    ...
    it("builds a cookie header from param cookies only", () => {
      const result = buildRequestCookieHeader({
        paramCookies: [
          createCookie("localCookie", "foo", { domain: undefined }),
        ],
        globalCookies: [],
        env: {},
        disabledGlobalCookies: {},
    ...
    it("builds a cookie header from global cookies only", () => {
      const result = buildRequestCookieHeader({
        paramCookies: [],
        globalCookies: [
          createCookie("globalCookie", "bar", { domain: "example.com" }),
        ],
        env: {},
  ...
  describe("domain filtering", () => {
    ...
    it("includes cookies with wildcard domain for subdomains", () => {
      const result = buildRequestCookieHeader({
        paramCookies: [],
        globalCookies: [
          createCookie("wildcardCookie", "foo", { domain: ".scalar.com" }),
        ],
        env: {},
        disabledGlobalCookies: {},
        originalCookieHeader: undefined,
    ...
    it("excludes cookies without wildcard for subdomains", () => {
      const result = buildRequestCookieHeader({
        paramCookies: [],
        globalCookies: [
          createCookie("nonWildcardCookie", "foo", { domain: "scalar.com" }),
        ],
        env: {},
        disabledGlobalCookies: {},
        originalCookieHeader: undefined,
    ...
    it("includes cookies without domain specified", () => {
      const result = buildRequestCookieHeader({
        paramCookies: [],
        globalCookies: [
          createCookie("noDomainCookie", "foo", { domain: undefined }),
        ],
        env: {},
        disabledGlobalCookies: {},
        originalCookieHeader: undefined,
  ...
  describe("disabled cookies", () => {
    it("excludes disabled global cookies", () => {
      const result = buildRequestCookieHeader({
        paramCookies: [],
        globalCookies: [
          createCookie("enabledCookie", "foo", {
            domain: "example.com",
            isDisabled: false,
          }),
          createCookie("disabledCookie", "bar", {
            domain: "example.com",
            isDisabled: true,
          }),
        ],
    ...
    it("includes cookies with isDisabled undefined", () => {
      const result = buildRequestCookieHeader({
        paramCookies: [],
        globalCookies: [
          createCookie("undefinedDisabledCookie", "foo", {
            domain: "example.com",
  ...
  describe("environment variable replacement", () => {
    ...
    it("handles multiple environment variables in a single value", () => {
      const result = buildRequestCookieHeader({
        paramCookies: [],
        globalCookies: [
          createCookie("session", "{{userId}}-{{timestamp}}", {
            domain: "example.com",
          }),
        ],
    ...
    it("leaves unreplaced variables when env is missing", () => {
      const result = buildRequestCookieHeader({
        paramCookies: [],
        globalCookies: [
          createCookie("token", "{{missingVar}}", { domain: "example.com" }),
        ],
        env: {},
        disabledGlobalCookies: {},
        originalCookieHeader: undefined,
  ...
  describe("complex scenarios", () => {
    it("combines param cookies, global cookies, and original header", () => {
      const result = buildRequestCookieHeader({
        paramCookies: [
          createCookie("localCookie", "local", { domain: undefined }),
        ],
        globalCookies: [
          createCookie("globalCookie", "global", { domain: "example.com" }),
        ],
        env: {},
        disabledGlobalCookies: {},
        originalCookieHeader: "existing=value",
    ...
    it("handles all filtering conditions together", () => {
      const result = buildRequestCookieHeader({
        paramCookies: [
          createCookie("paramCookie", "param", { domain: undefined }),
        ],
        globalCookies: [
          createCookie("validCookie", "valid", {
            domain: "example.com",
            isDisabled: false,
          }),
          createCookie("disabledCookie", "disabled", {
            domain: "example.com",
            isDisabled: true,
          }),
          createCookie("wrongDomainCookie", "wrong", {
            domain: "scalar.com",
            isDisabled: false,
          }),
          createCookie("", "empty", {
          ...
          createCookie("{{envCookie}}", "{{envValue}}", {
            domain: "example.com",
            isDisabled: false,
          }),
        ],
      ...
      expect(result).toMatchObject({
        name: "X-Scalar-Cookie",
        value:
          "original=header; validCookie=valid; envName=envVal; paramCookie=param",
      });
    });
  });
  ...
  describe("URL variations", () => {
    it("handles localhost URLs", () => {
      const result = buildRequestCookieHeader({
        paramCookies: [],
        globalCookies: [
          createCookie("localCookie", "value", { domain: "localhost" }),
        ],
        env: {},
        disabledGlobalCookies: {},
        originalCookieHeader: undefined,
    ...
    it("handles IP address URLs", () => {
      const result = buildRequestCookieHeader({
        paramCookies: [],
        globalCookies: [
          createCookie("ipCookie", "value", { domain: "127.0.0.1" }),
        ],
        env: {},
        disabledGlobalCookies: {},
        originalCookieHeader: undefined,
  ...
  describe("path filtering", () => {
    ...
    it("filters multiple cookies by path correctly", () => {
      const result = buildRequestCookieHeader({
        paramCookies: [],
        globalCookies: [
          createCookie("rootCookie", "root", {
            domain: "example.com",
            path: "/",
          }),
          createCookie("apiCookie", "api", {
            domain: "example.com",
            path: "/api",
          }),
          createCookie("adminCookie", "admin", {
            domain: "example.com",
            path: "/admin",
          }),
          createCookie("apiUsersCookie", "apiusers", {
            domain: "example.com",
            path: "/api/users",
          }),
        ],
  ...
  describe("ordering", () => {
    it("places global cookies before param cookies", () => {
      ...
      expect(result).toMatchObject({
        name: "Cookie",
        value:
          "global1=gvalue1; global2=gvalue2; param1=pvalue1; param2=pvalue2",
      });
    });
    ...
    it("places original header first", () => {
      const result = buildRequestCookieHeader({
        ...
        globalCookies: [
          createCookie("global", "gvalue", { domain: "example.com" }),
        ],
        env: {},
        disabledGlobalCookies: {},
      ...
      expect(result).toMatchObject({
        name: "Cookie",
        value:
          "original1=value1; original2=value2; global=gvalue; param=pvalue",
      });
    });
  });
