...
describe("sendRequest", () => {
  ...
  describe("plugin hooks", () => {
    ...
    it("executes beforeRequest hook before making the request", async () => {
      ...
      const beforeRequestMock = vi
        .fn()
        .mockImplementation(async (req: { request: Request }) => {
      ...
      const responseData = JSON.parse(result.response.data as string);
      expect(responseData.headers[HEADER_NAME.toLowerCase()]).toBe(
        HEADER_VALUE,
      );
    });
