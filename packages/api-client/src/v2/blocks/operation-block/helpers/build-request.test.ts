...
describe("buildRequest", () => {
  ...
  it("encodes forward slashes in path parameters", () => {
    ...
    expect(result?.request.url).toBe(
      "https://api.example.com/files/folder%2Ffile.txt",
    );
  });
  ...
  it("encodes multiple special characters in path parameters", () => {
    ...
    expect(error).toBe(null);
    expect(result?.request.url).toBe(
      "https://api.example.com/resources/user%2F123%3Ffilter%3Dactive%23top",
    );
  });
  ...
  it("encodes hash symbols in path parameters", () => {
    ...
    expect(error).toBe(null);
    expect(result?.request.url).toBe(
      "https://api.example.com/tags/%23important",
    );
  });
  ...
  it("encodes question marks in path parameters", () => {
    ...
    expect(error).toBe(null);
    expect(result?.request.url).toBe(
      "https://api.example.com/search/what%3Fwhy",
    );
  });
  ...
  it("encodes environment variables with special characters in path parameters", () => {
    ...
    expect(error).toBe(null);
    expect(result?.request.url).toBe(
      "https://api.example.com/files/folder%2Fsubfolder%2Ffile.txt",
    );
  });
  ...
  it("encodes multiple path parameters", () => {
    ...
    expect(error).toBe(null);
    expect(result?.request.url).toBe(
      "https://api.example.com/users/user%23123/files/docs%2Freadme.md",
    );
  });
  ...
  it("sends query parameters as arrays", () => {
    ...
    expect(error).toBe(null);
    expect(result?.request.url).toBe(
      "https://api.example.com/search?foo=foo&foo=bar",
    );
  });
  ...
  it("merges query parameters from server url", () => {
    ...
    expect(error).toBe(null);
    expect(result?.request.url).toBe(
      "https://api.example.com/api?orange=apple&example=parameter&foo=bar",
    );
  });
  ...
  it("adds User-Agent header in Electron", () => {
    ...
    expect(error).toBe(null);
    expect(result?.request.headers.get("X-Scalar-User-Agent")).toBe(
      "custom-user-agent",
    );
  ...
  it("sends cookies", () => {
    ...
    const cookieHeader =
      result?.request.headers.get("X-Scalar-Cookie") ||
  ...
  it("uses custom cookie header in Electron", () => {
    ...
    expect(error).toBe(null);
    expect(result?.request.headers.get("X-Scalar-Cookie")).toContain(
      "session=abc123",
    );
  ...
  describe("authentication", () => {
    ...
    it("adds apiKey auth in query", () => {
      ...
      expect(error).toBe(null);
      expect(result?.request.url).toBe(
        "https://api.example.com/users?api_key=test-key",
      );
    });
    ...
    it("handles complex auth with multiple schemes", () => {
      ...
      expect(result?.request.url).toBe(
        "https://api.example.com/users?api_key=xxxx",
      );
    });
  ...
  describe("request body", () => {
    ...
    it("deletes Content-Type header for FormData body", () => {
      ...
      expect(result?.request.headers.get("Content-Type")).toContain(
        "multipart/form-data",
      );
    });
  ...
  describe("example keys", () => {
    it("uses specified example key", () => {
      ...
      expect(error).toBe(null);
      expect(result?.request.url).toBe(
        "https://api.example.com/search?foo=custom-value",
      );
    });
