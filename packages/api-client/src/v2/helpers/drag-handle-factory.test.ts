...
const getSidebarState = (store: ReturnType<typeof createWorkspaceStore>) => {
  const entries = computed(() => {
    const order =
      store.workspace["x-scalar-order"] ??
      Object.keys(store.workspace.documents);
    return order
...
describe("handleDragEnd", () => {
  ...
  it("successfully reorders documents when moving after another document", async () => {
    ...
    const result = handleDragEnd(
      ...
      { id: "doc-2", parentId: null, offset: "after" },
    );
  ...
  it("returns false when trying to drop document into another document", () => {
    ...
    const result = handleDragEnd(
      { id: "doc-1", parentId: null },
      { id: "doc-1", parentId: null, offset: "into" },
    );
  ...
  it("returns false when dragging document over non-document item", () => {
    const sidebarState = getSidebarState(store);
    const navigation =
      store.workspace.documents["doc-1"]?.["x-scalar-navigation"];
    const tagId = navigation?.children?.find(
    ...
    const result = handleDragEnd(
      { id: "doc-1", parentId: null },
      { id: tagId, parentId: null, offset: "before" },
    );
  ...
  it("successfully reorders tags within the same parent", () => {
    ...
    const navigation =
      store.workspace.documents["doc-1"]?.["x-scalar-navigation"];
    const tags =
      navigation?.children?.filter((child) => child.type === "tag") ?? [];
    ...
    // Check the order of the tags
    const updatedSidebarStructre =
      store.workspace.documents["doc-1"]?.["x-scalar-navigation"];
    assert(updatedSidebarStructre, "Updated sidebar structure is required");
    const updatedOrder = updatedSidebarStructre.children?.map(
      (child) => child.id,
    );
  ...
  it("returns false when trying to reorder tags from different parents", async () => {
    ...
    const tag1 = nav1?.children?.find(
      (child) => child.type === "tag" && child.name === "users",
    );
    const tag2 = nav2?.children?.find(
      (child) => child.type === "tag" && child.name === "items",
    );
    ...
    const result = handleDragEnd(
      { id: tag1.id, parentId: null },
      { id: tag2.id, parentId: null, offset: "before" },
    );
  ...
  it("returns false when trying to drop tag into parent", () => {
    ...
    const result = handleDragEnd(
      { id: tag.id, parentId: null },
      { id: tag.id, parentId: null, offset: "into" },
    );
  ...
  it("successfully reorders operations within the same parent", () => {
    ...
    const tag = navigation?.children?.find(
      (child) => child.type === "tag" && child.name === "users",
    );
    ...
    const operations =
      tag.children?.filter((child) => child.type === "operation") ?? [];

    expect(operations.map((it) => it.id)).toEqual([
      "doc-1/tag/users/GET/users",
      "doc-1/tag/users/POST/users",
    ]);
    ...
    const result = handleDragEnd(
      { id: opPost.id, parentId: null },
      { id: opGet.id, parentId: null, offset: "before" },
    );

    expect(result).toBe(true);
    const tagObject = store.workspace.documents["doc-1"]?.tags?.find(
      (it) => it.name === "users",
    );
    ...
    const updatedTag =
      updatedNavigation.children?.filter(
        (child) => child.type === "tag" && child.name === "users",
      ) ?? [];
  ...
  it("returns false when trying to reorder operations from different parents", () => {
    ...
    const navigation =
      store.workspace.documents["doc-1"]?.["x-scalar-navigation"];
    const tagUsers = navigation?.children?.find(
      (child) => child.type === "tag" && child.name === "users",
    );
    const tagPets = navigation?.children?.find(
      (child) => child.type === "tag" && child.name === "pets",
    );
    ...
    const result = handleDragEnd(
      { id: op1.id, parentId: null },
      { id: op2.id, parentId: null, offset: "before" },
    );
  ...
  it("successfully moves an operation containing a circular reference to another tag", async () => {
    ...
    const sidebarState = getSidebarState(store);
    const navigation =
      store.workspace.documents["circular-doc"]?.["x-scalar-navigation"];

    const tagPeople = navigation?.children?.find(
      (child) => child.type === "tag" && child.name === "people",
    );
    const tagAnimals = navigation?.children?.find(
      (child) => child.type === "tag" && child.name === "animals",
    );
    ...
    const operation = tagPeople.children?.find(
...
describe("isDroppable", () => {
  ...
  it("returns true when dragging document over another document with reorder offset", async () => {
    ...
    const result = isDroppable(
      { id: "doc-1", parentId: null },
      { id: "doc-2", parentId: null, offset: "before" },
    );
  ...
  it("returns false when trying to drop document into another document", async () => {
    ...
    const result = isDroppable(
      { id: "doc-1", parentId: null },
      { id: "doc-2", parentId: null, offset: "into" },
    );
  ...
  it("returns false when dragging document over non-document", () => {
    ...
    const result = isDroppable(
      { id: "doc-1", parentId: null },
      { id: tag.id, parentId: null, offset: "before" },
    );
  ...
  it("returns false when trying to reorder operations with different parents", () => {
    ...
    const op1 =
      tagUsers && "children" in tagUsers
        ? tagUsers.children?.find((child) => child.type === "operation")
        : undefined;
    const op2 =
      tagPets && "children" in tagPets
        ? tagPets.children?.find((child) => child.type === "operation")
        : undefined;
  ...
  it("returns false when dragging example item", () => {
    ...
    const example = operation.children?.find(
