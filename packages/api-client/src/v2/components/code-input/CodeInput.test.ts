...
describe('CodeInput', () => {
  ...
  it('serializes array values to JSON for display', async () => {
    const wrapper = mount(CodeInput, {
      props: {
    ...
    // The CodeMirror content should contain the JSON array
    const codeMirrorElement = wrapper.find('.cm-content')
    expect(codeMirrorElement.exists()).toBe(true)
  })

  it('handles string array values', async () => {
    const wrapper = mount(CodeInput, {
      props: {
  ...
  it('handles number array values', async () => {
    const wrapper = mount(CodeInput, {
      props: {
        modelValue: [1, 2, 3],
        environment: undefined,
    ...
    await wrapper.vm.$nextTick()
    expect(wrapper.vm.modelValue).toEqual([1, 2, 3])
  })

  it('handles boolean array values', async () => {
    const wrapper = mount(CodeInput, {
      props: {
    ...
    await wrapper.vm.$nextTick()
    expect(wrapper.vm.modelValue).toEqual([true, false, true])
  })

  it('handles mixed type array values', async () => {
    const wrapper = mount(CodeInput, {
      props: {
        modelValue: ['string', 123, true],
        environment: undefined,
      },
    })

    await wrapper.vm.$nextTick()
    expect(wrapper.vm.modelValue).toEqual(['string', 123, true])
  })
  ...
  it('emits array values when JSON array is edited', async () => {
    const wrapper = mount(CodeInput, {
      props: {
        modelValue: ['initial'],
        environment: undefined,
    ...
    // Simulate editing the JSON array
    const newValue = '["updated", "values"]'
    wrapper.vm.handleChange(newValue)

    // Get the last emitted value (component may emit on mount)
    const emitted = wrapper.emitted('update:modelValue')
    expect(emitted).toBeDefined()
    expect(emitted?.[emitted.length - 1]).toEqual([newValue])
  })

  it('handles non-array values correctly', async () => {
    const wrapper = mount(CodeInput, {
      props: {
        modelValue: 'simple string',
        environment: undefined,
      },
    })

    await wrapper.vm.$nextTick()
    expect(wrapper.vm.modelValue).toBe('simple string')
  })
