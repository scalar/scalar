...
describe("upgradeFromTwoToThree", () => {
  ...
  it("rewrites $refs to definitions", () => {
    ...
    expect(
      result.paths?.["/planets"]?.get?.responses?.["200"]?.content[
        "application/json"
      ].schema.$ref,
  ...
  describe("parameter defaults", () => {
    it("preserves default on operation-level (inline) parameters", () => {
              ...
              parameters: [
                {
                  ...
                  required: true,
                  default:
                    "Token token=LOCATION_KEY_GOES_HERE, btoken=BUSINESS_KEY_GOES_HERE",
                  description:
                    "API key and business key as Authorization header.",
                },
                {
                  in: "header",
                  ...
                  type: "string",
                  description:
                    "Short code for locale variant (e.g., fr-ca, es-ES).",
                  default: "en",
                },
              ],
      ...
      expect(authParam.schema?.default).toBe(
        "Token token=LOCATION_KEY_GOES_HERE, btoken=BUSINESS_KEY_GOES_HERE",
      );
      ...
      const result: OpenAPIV3.Document = upgradeFromTwoToThree({
        ...
        parameters: {
          Authorization: {
            ...
            type: "string",
            description:
              "API key and business key (UUID) as the Authorization header.",
            default:
              "Token token=LOCATION_KEY_GOES_HERE, btoken=BUSINESS_KEY_GOES_HERE",
          },
          "Content-Type": {
  ...
  it("upgrades parameters defined globally and path wide - body and formData", () => {
    ...
    expect(
      result.paths?.["/planets/{planetId}"]?.post?.requestBody,
    ).toStrictEqual({
      content: {
        "multipart/form-data": {
          schema: {
  ...
  it("transforms x-example on parameters to examples with value structure", () => {
    ...
    const headerParameter = result.paths?.["/test"]?.post
    ...
    expect(
      (headerParameter as Record<string, unknown>)["x-example"],
    ).toBeUndefined();
  });
  ...
  it("transforms x-examples on body parameters with direct value to examples with default key", () => {
    ...
    const requestBody = result.paths?.["/test"]?.post
  ...
  it("transforms x-examples on body parameters with named examples without value wrappers", () => {
    const result: OpenAPIV3.Document = upgradeFromTwoToThree({
      swagger: "2.0",
      info: {
        title: "x-examples named examples without value wrappers",
        version: "1.0",
      },
      paths: {
        "/test": {
  ...
  it("ignores x-example when it contains a non-object value like a string", () => {
    ...
    const headerParameter = result.paths?.["/test"]?.get
      ?.parameters?.[0] as OpenAPIV3.ParameterObject;
  ...
  it("correctly wraps parameter x-examples with data that has a value property", () => {
    ...
    const result: OpenAPIV3.Document = upgradeFromTwoToThree({
      swagger: "2.0",
      info: {
        title: "parameter x-examples with value property test",
        version: "1.0",
      },
      paths: {
        "/test": {
  ...
  it("transforms response examples from Swagger 2.0 to OpenAPI 3.0 format", () => {
    ...
    const response200 = result.paths?.["/api/dashboard/locations"]?.get
    ...
    const response400 = result.paths?.["/api/dashboard/locations"]?.get
      ?.responses?.["400"] as OpenAPIV3.ResponseObject;
  ...
  it("upgrades tiny.yml example spec (minimal Swagger 2.0 with response schema and named examples)", () => {
    ...
    const responseOk = result.paths?.["/example"]?.get?.responses?.[
      "200"
    ...
    expect(
      responseOk.content?.["application/json"]?.examples?.Example?.value,
    ).toStrictEqual({
      user_id: 11111111,
    });
  ...
  it("transforms response examples with multiple media types", () => {
    const result: OpenAPIV3.Document = upgradeFromTwoToThree({
      swagger: "2.0",
      info: {
        title: "Response examples multiple media types test",
        version: "1.0",
      },
      produces: ["application/json", "application/xml"],
      paths: {
    ...
    const response200 = result.paths?.["/test"]?.get?.responses?.[
      "200"
    ] as OpenAPIV3.ResponseObject;
    expect(response200.content?.["application/json"]?.example).toStrictEqual({
      message: "Hello JSON",
    });
    expect(response200.content?.["application/xml"]?.example).toBe(
  ...
  it("transforms global responses defined in #/responses with examples", () => {
    ...
    expect(
      result.paths?.["/api/dashboard/locations"]?.get?.responses?.["200"],
    ).toStrictEqual({
      $ref: "#/components/responses/locations-response",
    });
    expect(
      result.paths?.["/api/dashboard/locations"]?.get?.responses?.["400"],
    ).toStrictEqual({
    ...
    const locationsResponse = result.components?.responses?.[
      "locations-response"
    ] as OpenAPIV3.ResponseObject;
    expect(locationsResponse.description).toBe(
      "Successful response with locations",
    );
    expect(
      locationsResponse.content?.["application/json"]?.schema,
    ).toStrictEqual({
      type: "object",
      properties: {
        locations: {
    ...
    expect(
      locationsResponse.content?.["application/json"]?.example,
    ).toStrictEqual({
      locations: ["New York", "Los Angeles"],
    });

    const errorResponse = result.components?.responses?.[
      "error-response"
    ] as OpenAPIV3.ResponseObject;
