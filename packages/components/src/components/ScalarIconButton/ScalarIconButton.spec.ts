...
describe('ScalarIconButton', () => {
  it('renders properly', async () => {
    ...
    expect(wrapper.html({ raw: true }))
      .toBe(`<button aria-disabled="false" aria-label="Logo button" class="scalar-button row items-center justify-center rounded font-medium scalar-button-icon text-sm scalar-button-ghost bg-transparent text-fore-3 active:text-fore-2 hocus:text-fore-2 h-10 w-10 p-2" title="Logo button" type="button"><div class="h-4 w-4"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 593 593" class="flow-icon" height="100%" width="100%">
