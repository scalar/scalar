...
describe("x-handler", () => {
    ...
    const document = {
      ...
      paths: {
        ...
        "/items/{id}": {
          put: {
            "x-handler":
              "return store.update('items', req.params.id, req.body);",
            responses: {
              "200": {
                description: "OK",
              },
    ...
    const document = {
      ...
      paths: {
        "/search": {
          get: {
            "x-handler":
              "return { query: req.query.q, limit: req.query.limit };",
            responses: {
              "200": {
                description: "OK",
              },
    ...
    const document = {
      ...
      paths: {
        "/items/{id}": {
          get: {
            "x-handler":
              "return { id: req.params.id, custom: 'handler-value' };",
            responses: {
              "200": {
                description: "OK",
                content: {
  ...
  describe("res object with example responses", () => {
    ...
    it("status code prioritizes semantically meaningful operations when multiple operations are performed", async () => {
      ...
      const getNotFoundResponse = await server.request(
