...
export function generateReqwest(isAsync: boolean, request?: Partial<HarRequest>, configuration?: PluginConfiguration) {
  // Defaults
  ...
  const blockingModifier = isAsync ? '' : 'blocking::'
  // Start building the Rust code
  let code = `let client = reqwest::${blockingModifier}Client::new();\n`
  ...
  code += ';\n'
  const asyncModifier = isAsync ? '.await' : ''
  code += `let response = request.send()${asyncModifier}?;\n`

  if (isJsonRequest) {
    code += `let body = response.json::<serde_json::Value>()${asyncModifier}?;\n`
  } else {
    code += `let body = response.text()${asyncModifier}?;\n`
  }
