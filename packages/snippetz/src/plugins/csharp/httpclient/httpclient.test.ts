...
using var response = await client.SendAsync(request);`);
  ...
  it("handles cookies", () => {
    ...
    expect(result).toContain(
      'request.Headers.TryAddWithoutValidation("Cookie", "sessionId=abc123; theme=dark");',
    );
  });
...
using var response = await client.SendAsync(request);`);
  ...
  it("handles duplicate headers by keeping the last value", () => {
    ...
    expect(result).toContain(
      'request.Headers.TryAddWithoutValidation("X-Custom", "value2");',
    );
  ...
  it("handles complex scenario with all features", () => {
    ...
    expect(result).toContain(
      'request.Headers.TryAddWithoutValidation("X-API-Version", "v1");',
    );
    expect(result).toContain(
