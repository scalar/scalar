...
describe("fsharpHttpclient.generate - headers", () => {
  ...
  it("adds multiple headers to HttpRequestMessage", () => {
    ...
    expect(result).toContain(
      'httpRequestMessage.Headers.Add("User-Agent", "TestAgent")',
    );
  });
...
describe("fsharpHttpclient.generate - postData", () => {
  ...
  it("adds multiple form-urlencoded params to HttpRequestMessage", () => {
    ...
    const result = fsharpHttpclient.generate(request, {});
    expect(result).toContain(
      "let formUrlEncodedContentDictionary = new Dictionary<string, string>()",
    );
    expect(result).toContain(
      'formUrlEncodedContentDictionary.Add("foo", "bar")',
    );
    expect(result).toContain(
      'formUrlEncodedContentDictionary.Add("baz", "qux")',
    );
    expect(result).toContain(
      "let content = new FormUrlEncodedContent(formUrlEncodedContentDictionary)",
    );
    expect(result).toContain("httpRequestMessage.Content <- content");
  });
  ...
  it("adds file param to MultipartFormDataContent", () => {
    ...
    expect(result).toContain(
      'content.Add(fileStreamContent_0, "test.txt", "test.txt")',
    );
    expect(result).toContain("httpRequestMessage.Content <- content");
  });
...
describe("fsharpHttpclient.generate - cookies", () => {
  ...
  it("adds cookies if cookies array is present and url is provided", () => {
    ...
    expect(result).toContain(
      'cookieContainer.Add(Uri("https://api.com/"), Cookie("foo", "bar"))',
    );
    expect(result).toContain(
      'cookieContainer.Add(Uri("https://api.com/"), Cookie("baz", "qux"))',
    );
...
describe("fsharpHttpclient.generate - edge cases", () => {
  ...
  it("handles special characters in URL", () => {
    ...
    const result = fsharpHttpclient.generate(request, {});
    expect(result).toContain(
      'new Uri("https://example.com/path with spaces/[brackets]")',
    );
  });
  ...
  it("handles special characters in query parameters", () => {
    ...
    const result = fsharpHttpclient.generate(request, {});
    expect(result).toContain(
      'new Uri("https://example.com?q=hello world & more&special=!@#$%^&*()")',
    );
  });
  ...
  it("handles URLs with dollar signs", () => {
    ...
    const result = fsharpHttpclient.generate(request, {});
    expect(result).toContain(
      'new Uri("https://example.com/path$with$dollars")',
    );
  });
  ...
  it("handles URLs with dollar signs in query parameters", () => {
    ...
    const result = fsharpHttpclient.generate(request, {});
    expect(result).toContain(
      'new Uri("https://example.com?price=$100&currency=USD$")',
    );
  });
  ...
  it("handles URLs with dollar signs in path and query", () => {
    ...
    const result = fsharpHttpclient.generate(request, {});
    expect(result).toContain(
      'new Uri("https://example.com/api$v1/prices?amount=$50.00"',
    );
  });
  ...
  it("handles multiple headers with same name", () => {
    ...
    const result = fsharpHttpclient.generate(request, {});
    expect(result).toContain(
      'httpRequestMessage.Headers.Add("X-Custom", "value1")',
    );
    expect(result).toContain(
      'httpRequestMessage.Headers.Add("X-Custom", "value2")',
    );
  });
