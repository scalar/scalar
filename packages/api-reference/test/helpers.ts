...
/**
 * The Storybook category every schema story lives under.
 *
 * Story ids are `${category}-${component}--${story}`, so this has to match the first segment of the
 * story `title` (e.g. `title: 'Schema/Schema'`). It is the one thing that differs from the
 * `@scalar/components` harness, which lives under the `Components` category.
 */
const STORY_CATEGORY = 'schema'
...
export const test = base.extend<ComponentTestOptions & ComponentTestFixtures>({
  ...
  openStory: [
    async ({ page, component: c, story: s, args, theme }, use, testInfo) => {
      ...
      const params = [
        ['viewMode', 'story'],
        ['id', `${STORY_CATEGORY}-${toSlug(component)}--${toSlug(story)}`],
