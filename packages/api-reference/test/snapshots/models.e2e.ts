...
toTest.forEach((source) => {
  ...
  test(source.title, async ({ page }) => {
    ...
    const discriminatorSelector = celestialBodyModel.getByRole('button', { name: 'One of' }).first()
    await discriminatorSelector.click()
    await celestialBodyModel.getByRole('option', { name: 'Satellite' }).first().click()
    await expect(discriminatorSelector).toContainText('Satellite')
  })
})
