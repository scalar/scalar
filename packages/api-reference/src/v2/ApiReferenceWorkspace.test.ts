...
describe('ApiReferenceWorkspace', () => {
  ...
  describe('relative URL handling', async () => {
    beforeEach(() => {
    ...
    it('resolves relative URLs in single document configuration', async () => {
      const fn = vi.fn()
      wrapper = mount(ApiReferenceWorkspace, {
        props: {
          configuration: {
      ...
      expect(fn).toHaveBeenCalledWith('https://example.com/docs/openapi.json', undefined)
    })

    it('resolves relative URLs in sources array configuration', async () => {
      const fn = vi.fn()
      wrapper = mount(ApiReferenceWorkspace, {
        props: {
          configuration: {
            ...
            sources: [
              {
                name: 'API v1',
                url: 'v1/openapi.json', // relative URL
              },
              {
                name: 'API v2',
                url: '../spec/v2/openapi.yaml', // relative URL with parent directory
              },
            ],
            fetch: async (...args) => {
              fn(...args)
      ...
      // Should resolve both relative URLs to absolute URLs
      expect(fn).toHaveBeenCalledWith('https://example.com/docs/v1/openapi.json', undefined)
      expect(fn).toHaveBeenCalledWith('https://example.com/spec/v2/openapi.yaml', undefined)
    })
  })
