...
describe("createApiReference", () => {
  ...
  it("handles string selectors for mounting", () => {
    ...
    expect(apiReference.app.mount).toBeDefined();
    expect(apiReference.updateConfiguration).toBeDefined();
    expect(document.getElementById("mount-point")?.innerHTML).toContain(
      "Powered by Scalar",
    );
  });
  ...
  it("allows updating configuration after creation", async () => {
    ...
    const newConfig = {
      content:
        '{"openapi":"3.1.0", "info": {"title": "Updated API", "version": "1.0.0"}}',
    };
  ...
  it("updates the operations when the configuration changes", async () => {
    ...
    expect(
      document.getElementById("updated-api/tag/test/GET/test"),
    ).not.toBeNull();
    expect(
      document.getElementById("updated-api/tag/test/GET/test")?.innerHTML,
    ).toContain("New Operation");
    ...
    await sleep(1000);
    expect(
      document.getElementById("updated-api/tag/test/POST/test"),
    ).not.toBeNull();
    expect(
      document.getElementById("updated-api/tag/test/POST/test")?.innerHTML,
    ).toContain("Even newer operation");
...
describe("getConfigurationFromDataAttributes", () => {
  ...
  it("handles deprecated data-spec attribute with warning", () => {
    ...
    expect(consoleWarnSpy).toHaveBeenCalledWith(
      expect.stringContaining("The [data-spec] HTML API is deprecated"),
    );
  });
  ...
  it("handles deprecated data-spec-url attribute with warning", () => {
    ...
    expect(consoleWarnSpy).toHaveBeenCalledWith(
      expect.stringContaining("The [data-spec-url] HTML API is deprecated"),
    );
  });
