...
describe("Schema", () => {
  ...
  describe("additionalProperties Vue prop", () => {
    ...
    it("renders additional properties with noncollapsible prop set to true", async () => {
      ...
      const schemaProperties = wrapper.findAllComponents({
        name: "SchemaProperty",
      });
      const additionalProperty = schemaProperties.find(
        (prop) => prop.props("variant") === "additionalProperties",
      );
          ...
          schema: coerceValue(SchemaObjectSchema, {
            ...
            properties: {
              closed_on: {
                ...
                description:
                  "The date the object was closed in YYYY-MM-DD or ISO 8601 format.",
              },
              status: {
                type: "string",
