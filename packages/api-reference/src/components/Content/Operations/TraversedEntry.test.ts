...
const { default: TraversedEntryComponent } = await import(
  "./TraversedEntry.vue"
);
...
): ComponentProps<typeof TraversedEntryComponent> => ({
  entries,
...
describe("operation rendering", () => {
  ...
  it("renders multiple operations correctly", () => {
    ...
    const operationComponents = wrapper.findAllComponents({
      name: "Operation",
    });
