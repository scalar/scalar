...
describe("ApiReference Configuration Tests", () => {
  it("default configuration values", async () => {
    ...
    expect(
      wrapper.findComponent({ name: "Content" }).text().includes("Get others"),
    ).toBe(false);
  ...
  it("all boolean and simple configurations", async () => {
    ...
    expect(
      wrapper
        .findComponent({ name: "Content" })
        .text()
        .includes("1234getUserById5678"),
    ).toBe(true);
  ...
  it("all callbacks and complex configurations", async () => {
    ...
    const wrapper = mountComponent({
      props: {
        configuration: {
          ...
          onShowMore,
          onDocumentSelect,
          generateHeadingSlug: (heading: { slug: string }) =>
            `custom-test-slug-${heading.slug}`,
    ...
    expect(ServerSelectorComponent.text().includes("api.example.com")).toBe(
      true,
    );

    // authentication: object
    const Auth = wrapper.findComponent({ name: "Auth" });
    const button = Auth.find(
      'button[data-testid="data-table-password-toggle"]',
    );
    await button.trigger("click");
    expect(Auth.text().includes("test-token")).toBe(true);

    // defaultHttpClient, hiddenClients, proxyUrl
    expect(wrapper.vm.workspaceStore.workspace["x-scalar-default-client"]).toBe(
      "node/axios",
    );
    const clientOptions = wrapper
      .findComponent({ name: "ClientSelector" })
      .props().clientOptions as ClientOptionGroup[];
    const clientKeysSet = new Set<string>(
      clientOptions.flatMap((group) =>
    ...
    expect(wrapper.vm.workspaceStore.workspace["x-scalar-active-proxy"]).toBe(
    ...
    const operationItem = sidebarItems.find((item) =>
      item.text().includes("Get posts"),
    );
    await operationItem?.trigger("click");
    expect(onSidebarClick).toHaveBeenCalled();
    expect(onSidebarClick).toHaveBeenCalledWith(
    ...
    const tagNames = wrapper.vm.sidebarItems
      .map((item) => item.title)
      .filter((text) =>
        ["Apples", "Bananas", "Monkeys", "Zebras"].includes(text),
      );
    ...
    const zebrasTag = wrapper.vm.sidebarItems.find(
      (item) => item.title === "Zebras",
    );
    ...
    if (showMoreButton.exists()) {
      ...
      expect(onShowMore).toHaveBeenCalledWith(
    ...
    // searchHotKey: f
    expect(
      wrapper.findComponent({ name: "SearchModal" }).props().modalState.open,
    ).toBe(false);
