...
describe("ServerSelector", () => {
  ...
  it("renders server variables form when variables are available", () => {
    ...
    expect(
      wrapper.findComponent({ name: "ServerVariablesForm" }).exists(),
    ).toBe(true);
  });
  ...
  it("responds to server variables form events", async () => {
    ...
    await variablesForm.vm.$emit("update:variable", {
      key: "environment",
      value: "staging",
    });
  ...
  it("reactively updates the store when the variables are updated", async () => {
    ...
    store.workspace.activeDocument!.servers![1]!.variables!.protocol!.default =
      "1234";
    await nextTick();
  ...
  it("renders with multiple servers and complex configuration", () => {
    ...
    expect(wrapper.findComponent({ name: "ScalarMarkdown" }).exists()).toBe(
      true,
    );

    // Should display the description
    expect(wrapper.text()).toContain(
      "Production server with detailed description",
    );
  });
