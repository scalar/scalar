...
describe("InfoLinks", () => {
  ...
  it("renders all components when all info properties are provided", () => {
    ...
    expect(wrapper.findComponent({ name: "TermsOfService" }).exists()).toBe(
      true,
    );
  });
  ...
  it("passes correct props to child components", () => {
    ...
    expect(
      wrapper.findComponent({ name: "ExternalDocs" }).props("value"),
    ).toEqual(mockExternalDocs);
    expect(wrapper.findComponent({ name: "Contact" }).props("value")).toEqual(
      mockContact,
    );
    expect(wrapper.findComponent({ name: "License" }).props("value")).toEqual(
      mockLicense,
    );
    expect(
      wrapper.findComponent({ name: "TermsOfService" }).props("value"),
    ).toEqual(mockTermsOfService);
  });
});
