...
const mountOperationWithConfig = (
  ...
  const props: ExtractComponentProps<typeof Operation> = {
    ...
    pathValue:
      overrides.pathValue !== undefined
        ? overrides.pathValue
        : defaultDocument.paths?.["/users/{userId}"],
...
describe("Operation", () => {
  ...
  it("overrides path parameters with operation parameters of the same name", () => {
            ...
            parameters: [
              {
                ...
                deprecated: true,
                description:
                  "Operation parameter description - should override",
              },
              {
                in: "query",
                name: "include",
    ...
    const pathParameter = operation.parameters.find(
      (p: any) => p.in === "path" && p.name === "userId",
    );
    ...
    expect(queryParameter.description).toBe(
      "Operation query parameter - should override",
    );
  });
  ...
  it("passes expandAllResponses config to OperationResponses component", () => {
                ...
                content: {
                  "application/json": {
                    schema: {
                      type: "string",
                      default:
                        "This is the testing string and it is in the document",
                    },
                  },
                },
              },
    ...
    expect(operationResponses.text()).toContain(
      "This is the testing string and it is in the document",
    );
  });
  ...
  it("passes operation-level server to ModernLayout", () => {
    ...
    const modernLayout = wrapper.findComponent({ name: "ModernLayout" });
    expect(modernLayout.exists()).toBe(true);
    const server = modernLayout.props("selectedServer") as
      | { url?: string }
      | undefined;
  ...
  describe("showOperationId", () => {
    ...
    describe("ClassicLayout", () => {
      it("shows operationId when showOperationId is true", () => {
        const wrapper = mountOperationWithConfig({
          options: { showOperationId: true, layout: "classic" },
        });
        const classicLayout = wrapper.findComponent({ name: "ClassicLayout" });
