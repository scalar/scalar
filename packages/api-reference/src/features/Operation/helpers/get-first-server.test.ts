...
describe("get-first-server", () => {
  describe("getFirstServer", () => {
    ...
    it("returns the first server from the first non-empty source", () => {
      ...
      const pathItemServers: ServerObject[] = [
        { url: "https://path.example.com", description: "Path server" },
      ];
    ...
    it("returns null when no servers have URLs", () => {
      // @ts-expect-error - Testing an invalid server object
      const servers: ServerObject[] = [
        { description: "No URL 1" },
        { description: "No URL 2" },
      ];
    ...
    it("handles mixed server types in the same call", () => {
      ...
      const serverArray: ServerObject[] = [
        { url: "https://array.example.com", description: "Array server" },
      ];
