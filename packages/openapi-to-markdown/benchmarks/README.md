...
The workloads include Scalar Galaxy 3.1, shared recursive schemas, long Markdown
descriptions, single-operation selection, and exporting every operation page.
They use local inputs; network reference-fetch latency is not measured. Synthetic
inputs are defined in `fixtures.mjs` and have no random values.
