...
describe("SidebarItem", () => {
  ...
  describe("rendering without children", () => {
    it("renders a simple item without children", () => {
      ...
      expect(wrapper.findComponent(ScalarSidebarItemComponent).exists()).toBe(
        true,
      );
  ...
  describe("rendering with children", () => {
    it("renders ScalarSidebarSection for group items with children", () => {
      ...
      expect(wrapper.findComponent(ScalarSidebarSection).text()).toContain(
        "User Operations",
      );
    });
  ...
  describe("drag and drop", () => {
    ...
    it("bubbles up onDragEnd event from child items", async () => {
      ...
      expect(wrapper.emitted("onDragEnd")?.[0]).toEqual([
        draggingItem,
        hoveredItem,
      ]);
    });
    ...
    it("inherits isDraggable prop from parent to child items when false", () => {
      ...
      const childComponents = childItems.filter(
        (w) => w.props("item").id !== "1",
      );
  ...
  describe("edge cases", () => {
    ...
    it("handles operations and webhooks with children in reference layout", () => {
      const item: Item = {
        ...
        path: "/users",
        children: [
          { id: "2", title: "Example 1", type: "example", name: "example1" },
        ],
      };
