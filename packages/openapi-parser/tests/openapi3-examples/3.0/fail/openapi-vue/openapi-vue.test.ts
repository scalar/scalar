...
describe.skip("openapi-vue", () => {
  it("openapi", async () => {
    ...
    const componentsErrors = result.errors.filter((e) =>
      e.path?.includes("/components"),
    );
    expect(componentsErrors.length).toBeGreaterThanOrEqual(2);
    expect(
      componentsErrors.some((e) => e.message.includes("definitions")),
    ).toBe(true);
    expect(
      componentsErrors.some((e) => e.message.includes("securityDefinitions")),
    ).toBe(true);
