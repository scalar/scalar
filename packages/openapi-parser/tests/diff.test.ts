...
const files = await fetch(
  ...
  .then((data) =>
    data
      ...
      .filter(
        (d) => d.name.startsWith("oas/") && !ignoreFiles.includes(d.name),
      ),
  );
...
const fetched = await Promise.all(
  set.map(async (file) => {
    const content = await fetch(
      `https://fixtures.staging.scalar.com/${file.name}`,
    )
      .then((r) => r.text())
      .catch(() => "");
    return {
...
console.log(
  `[openapi-parser-diff.test.ts] Successfully fetched ${fetched.length} files to test`,
);
