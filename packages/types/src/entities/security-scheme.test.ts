...
describe("Security Schemas", () => {
  ...
  describe("OAuth2 Schema", () => {
    ...
    it("should validate a valid OAuth2 authorization code flow schema", () => {
      ...
      expect(result.success).toBe(true);
      expect(
        result.data?.flows.authorizationCode?.["x-scalar-security-query"],
      ).toEqual({
        prompt: "consent",
      });
      expect(
        result.data?.flows.authorizationCode?.["x-scalar-security-body"],
      ).toEqual({
        audience: "foo",
      });
    });
    ...
    it("should apply default values", () => {
      ...
      expect(result.flows.implicit?.authorizationUrl).toBe(
        "http://localhost:8080",
      );
  ...
  describe("Security Requirement Schema", () => {
    it("should validate a valid security requirement", () => {
      ...
      const result =
        oasSecurityRequirementSchema.safeParse(securityRequirement);
      expect(result.success).toBe(true);
    });
