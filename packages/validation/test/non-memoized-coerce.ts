/**
 * Test-only reference frozen from main 216faf7b0b before score memoization.
 * Keep its scoring independent from production so differential tests detect branch drift.
 * It retains only active-pair cycle guards, never completed/contextual score caches.
 * This helper is outside src and is not built or publicly exported.
 */
import { isObject } from '../src/helpers/is-object'
import type { Schema } from '../src/schema'
import type { Static } from '../src/types'
 ...
 */
export const coerceWithoutScoreMemoization = <S extends Schema>(
  schema: S,
  value: unknown,
