...
describe("fixtures", () => {
  test.each(FIXTURES)("%s", async (file) => {
    ...
    const input = await fetch(
      `${BUCKET_URL}/packages/postman-to-openapi/input/${file}.json`,
    );
    const postman = await input.json();

    // openapi
    const output = await fetch(
      `${BUCKET_URL}/packages/postman-to-openapi/output/${file}.json`,
    );
...
describe("convert", () => {
  ...
  it("fails fast when string input is not valid JSON", () => {
    expect(() => convert('{"info": {"name": "Broken"')).toThrowError(
      /invalid postman collection json/i,
    );
  });

  it("errors when required collection info is missing", () => {
    expect(() => convert({} as PostmanCollection)).toThrowError(
      /missing required info/i,
    );
    expect(() =>
      convert({
  ...
  it("defaults requestBody content to text/plain when extractor returns empty content", () => {
    ...
    expect(
      postOperation?.requestBody && "content" in postOperation.requestBody,
    ).toBe(true);
