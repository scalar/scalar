...
describe("path-items", () => {
  ...
  it("processes request with auth and adds security scheme", () => {
    ...
    expect(result.paths["/users"]?.get?.security?.[0]).toHaveProperty(
      "bearerAuth",
    );
  });
...
| query  | page | Page number | 1       | integer | true |`,
    ...
    expect(result.paths["/users"]?.get?.description).toBe(
      "Get a list of users",
    );
  });
...
| query  | filter | Filter string | test | true |`,
  ...
  it("adds request body for GET request when body is provided", () => {
    ...
    expect(
      result.paths["/users"]?.get?.requestBody?.content?.["application/json"],
    ).toBeDefined();
  });
  ...
  it("allows GET requests to have request bodies", () => {
    ...
    expect(
      result.paths["/bedrock"]?.get?.requestBody?.content?.["application/json"],
    ).toBeDefined();
    const example = result.paths["/bedrock"]?.get?.requestBody?.content?.[
      "application/json"
  ...
  it("adds pre-request script extension", () => {
    ...
    expect(result.paths["/users"]?.get?.["x-pre-request"]).toBe(
  ...
  describe("disabled parameters and headers", () => {
    ...
    it("preserves disabled header parameters in operation", () => {
      ...
      const params = result.paths[pathKey!]?.get?.parameters;
      const customHeader = params?.find(
        (p: any) => p.name === "X-Custom-Header",
      );
      const anotherHeader = params?.find(
        (p: any) => p.name === "X-Another-Header",
      );
    ...
    it("preserves disabled formdata parameters in request body", () => {
      ...
      expect(
        schema?.properties?.description?.["x-scalar-disabled"],
      ).toBeUndefined();
    });
    ...
    it("handles mixed enabled and disabled parameters correctly", () => {
      ...
      const enabledHeader = params?.find(
        (p: any) => p.name === "X-Enabled-Header",
      );
      const disabledHeader = params?.find(
        (p: any) => p.name === "X-Disabled-Header",
      );
