...
describe("auth", () => {
  ...
  it("extracts OAuth2 authorization URL from auth attributes", () => {
    const auth = {
      ...
      oauth2: [
        ...
        {
          key: "accessTokenUrl",
          value: "https://login.example.com/oauth/token",
        },
      ],
    } satisfies Auth;
    ...
    expect(
      result.securitySchemes.oauth2Auth?.flows?.authorizationCode
        ?.authorizationUrl,
  ...
  it("extracts OAuth2 token URL from auth attributes", () => {
    ...
    expect(
      result.securitySchemes.oauth2Auth?.flows?.authorizationCode?.tokenUrl,
  ...
  it("extracts OAuth2 scopes from auth attributes", () => {
    ...
    const scopes =
      result.securitySchemes.oauth2Auth?.flows?.authorizationCode?.scopes;
  ...
  it("uses tokenUrl as fallback for accessTokenUrl", () => {
    const auth = {
      type: "oauth2",
      oauth2: [
        { key: "tokenUrl", value: "https://login.example.com/oauth/token" },
      ],
    } satisfies Auth;
  ...
  it("throws when authentication type is unsupported", () => {
    ...
    expect(() => processAuth(auth)).toThrowError(
