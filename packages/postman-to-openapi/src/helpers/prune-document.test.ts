...
describe("pruneDocument", () => {
  ...
  it("removes undefined values from request bodies and responses", () => {
    ...
    expect(
      requestBody?.content?.["application/json"]?.encoding,
    ).toBeUndefined();
    expect("examples" in requestBody!.content!["application/json"]!).toBe(
      false,
    );
    expect("encoding" in requestBody!.content!["application/json"]!).toBe(
      false,
    );
  ...
  it("removes undefined values from deeply nested structures", () => {
    ...
    expect(
      "examples" in operation!.responses!["200"]!.content!["application/json"]!,
    ).toBe(false);
  });
