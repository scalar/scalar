...
describe("ReactRenderer", { retry: 3 }, () => {
  const TestComponent: FC<{ message?: string }> = ({
  ...
  it("transforms kebab-case props to camelCase", async () => {
    const CaseTestComponent: FC<{ customProp?: string }> = ({
      customProp = "",
...
describe("ReactConnector", { retry: 3 }, () => {
  ...
  it("transforms props correctly", async () => {
    const TestComponent: FC<{ testProp: string }> = ({ testProp }) => (
      <div data-testid="test">{testProp}</div>
    );
