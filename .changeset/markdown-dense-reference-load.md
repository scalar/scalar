...
References to schemas that coercion drops, such as a root-level `definitions` block, now link to a cast copy of the schema, and their anchors resolve against the resource that contains them. A `$ref: '#'` inside a schema with an `$id` now links to that schema.
