...
The query string is now split off the path key and matched against the request instead. A key that carries one is registered before the key it shares a route with, so `/v1/messages?beta=true` answers a request carrying `beta=true` and `/v1/messages` answers the rest. Because such a query string tells two operations apart rather than describing something a client has to send, a route whose keys all carry one still answers a request without it.
