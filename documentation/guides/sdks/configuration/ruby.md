...
Registry accounts, secrets, and first-release setup: [Ruby publishing](../publishing/ruby.md).
...
Defaults to a `snake_case` name derived from `organization.name`, which is what RubyGems prefers for a gem that occupies a single namespace: a dash is read there as a namespace separator, so `acme-widget` conventionally means the `Acme::Widget` gem whose entrypoint is `lib/acme/widget.rb`, and the generated gem is flat.

A dash spelled here is preserved, so a gem already published under a kebab-case name (`orb-billing`) keeps it. Each dash-delimited segment is normalized on its own, so other separators and casing still fold into `snake_case` — `context.dev` becomes `context_dev` and `MyGem` becomes `my_gem`. Changing this value renames the published gem and the release-please component, so pin it to the current name for a gem that has already shipped.
...
RubyGems publishing configuration.

### rubygems

**Type:** `boolean | object`

RubyGems publishing configuration for Ruby gems. `true` publishes with the defaults; an object tunes them.

Releases use [RubyGems trusted publishing](https://guides.rubygems.org/trusted-publishing/) (OIDC) unless `authMethod` says otherwise: the publish job exchanges its GitHub Actions id-token for a short-lived, gem-scoped rubygems.org API key, so no `RUBYGEMS_API_KEY` secret is stored. Register this repository and its release workflow as a *pending* publisher at [rubygems.org/profile/oidc/pending_trusted_publishers](https://rubygems.org/profile/oidc/pending_trusted_publishers), naming the destination repository, the workflow file the publish job runs from — `release-please.yml`, the workflow the automated release publishes from (register `sdk-release.yml` as a second publisher only if the manual re-publish workflow is used) — and the `releaseEnvironment` when one is set. A pending publisher names the gem up front and becomes an ordinary publisher on the first successful push, which is what a gem with no releases yet needs: the gem's own **Trusted publishers** page (`https://rubygems.org/gems/<gem>/trusted_publishers`) is served only for a gem that already exists and is owned by you, and that page is where an already-released gem is configured.

Set `authMethod` to `access-token` to publish with a [rubygems.org API key](https://rubygems.org/profile/api_keys) that has the *Push rubygem* scope, stored as the `RUBYGEMS_API_KEY` repository secret, instead. That key is only usable unattended at the right [MFA level](https://guides.rubygems.org/setting-up-multifactor-authentication/): at **UI and API** every push is answered with an OTP prompt no workflow can satisfy, so either publish over OIDC or move the account to **UI and gem signin** and leave MFA off this key. Both mechanisms are fully supported, and switching to OIDC is a config change and nothing else, which is the usual adoption order: a gem with no trusted publisher registered yet can release with `authMethod` set to `access-token`, then drop the override (or set `oidc` explicitly) once the publisher is registered on rubygems.org. Repository secrets live under **Settings → Secrets and variables → Actions** in the destination repository; an environment secret of the same name overrides one there when `releaseEnvironment` is set.
