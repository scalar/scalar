...
Registry accounts, secrets, and first-release setup: [PHP publishing](../publishing/php.md).

## packageName

**Type:** `string`

PHP package namespace/name.

## composerPackageName

**Type:** `string`

PHP Composer and Packagist package name.
...
Packagist publishing configuration.

### packagist

**Type:** `boolean | object`

Packagist publishing configuration for PHP packages. `true` publishes with the defaults; an object tunes them.

Packagist reads versions from the repository's git tags, so a release is the `vX.Y.Z` tag and nothing else: there is no upload step, no secret, and `authMethod` is unused. Submit the destination repository once at [packagist.org/packages/submit](https://packagist.org/packages/submit) and let Packagist install its GitHub hook (or enable the [GitHub integration](https://packagist.org/profile/) on your account); every later tag is picked up automatically.
...
Composer repository URL for PHP package publishing.
