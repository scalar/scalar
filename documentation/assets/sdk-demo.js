...
const initSdkDemo = (root) => {
  ...
  /* Both rebuilt whenever a run starts, so they reflect the targets installed
   * then. A target added mid-run belongs to the next run, not this one. */
  let buildSteps = buildLog(state.installed)
  let buildTargets = [...state.installed]
  ...
  const renderPanel = () => {
    ...
    /* Point whichever panel is showing back at the tab that selected it. */
    const panel = showFiles ? nodes.files : nodes.code
    panel?.setAttribute('aria-labelledby', `sdk-demo-tab-${state.tab}`)
  ...
  const reset = () => {
    ...
    setShareOpen(false)
    if (nodes.siteTab) {
      delete nodes.siteTab.dataset.sdkDemoTabOpen
  ...
  /* Load the embed for a tab on first visit, and never before. Closing the demo
   * drops both, so reopening on an embed tab comes back through here. */
  const loadEmbed = (page) => {
  ...
  const filterTabs = (query) => {
    ...
    nodes.pageTabs.forEach((tab) => {
      /* A tab that has not been opened yet is not a tab, so searching or
       * clearing the search must never be what reveals it. */
      if (tab.dataset.sdkDemoTabOpen !== 'true') {
        tab.hidden = true
        return
      }
  ...
  nodes.newTabs.forEach((button) => {
    button.addEventListener('click', () => {
      if (nodes.siteTab) {
        nodes.siteTab.dataset.sdkDemoTabOpen = 'true'
  ...
  /* Where focus was before the sheet took it, so closing can hand it back. */
  ...
  const setShareOpen = (open) => {
    ...
    if (open) {
      setMinimized(false)
      hideHint()
      /* aria-modal hides the rest of the page from assistive tech, so focus has
       * to move inside the sheet or there is nothing left to read. */
      shareReturnFocus = document.activeElement
      nodes.shareSheet?.querySelector('button, [href]')?.focus()
    } else {
      if (shareReturnFocus?.isConnected) {
  ...
  nodes.reopen?.addEventListener('click', () => {
    setClosed(false)
    /* Closing dropped the embeds, so bring back the tab the reader left on. */
    loadEmbed(root.dataset.sdkDemoPage)
    showHint()
  })
  ...
  nodes.targets?.addEventListener('click', (event) => {
    ...
    closeAddMenu()
    render()
    /* render() replaces these nodes, so follow focus to the card that replaced
     * the one the reader activated. */
    qs(root, `[data-target="${state.selected}"]`)?.focus()
  })
