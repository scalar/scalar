...
`x-scalar-examples` and `x-stainless-examples` share the same shape: an example (or array of examples) with an optional `title` and a `request` map of source code keyed by language. The `title` is used as the label in the picker.
