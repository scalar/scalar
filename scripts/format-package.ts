...
async function formatPackage(filepath: string, packageType: PackageType) {
  ...
  validateNxConfig(formattedData.nx, packageType)
...
/** Ensure package nx config is correct */
function validateNxConfig(
  nxEntry: Record<string, unknown> & { tags?: string[] },
  packageType: PackageType,
) {
  if (!nxEntry.tags) nxEntry.tags = []

  if (packageType === 'packages' && !nxEntry.tags.includes('scope:package')) {
